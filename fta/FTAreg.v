...
[[
(p:(cpoly CC))(monic n p)->(c:IR)((AbsCC (p!Zero)) [<] c)->{z:CC &
...
Record Knes_tup : Type := 
  {z_el :> CC; c_el : IR; Kt_prop : AbsCC p ! z_el[<]c_el}.
...
Lemma sK_it :
 forall tup : Knes_tup, AbsCC (Knes_fun tup[-]tup)[^]n[<=]c_el tup.
Proof.
intro tup.
...
Let qmonz : q[-]One[#]Zero := qltone IR q qlt1.
Variable e : IR.
Variable zlte : Zero[<]e.

Lemma N_exists :
 {N : nat |
 forall m : nat, N <= m -> (q[^]m[-]q[^]N[/] q[-]One[//]qmonz)[*]c[<=]e}.
Proof.
cut (Zero[<]One[-]q).
intro H0.
cut (One[-]q[#]Zero).
...
Let nrtqmonz : nrtq[-]One[#]Zero := qltone IR nrtq nrtqlt1.

Definition zlt_nrtq : Zero[<=]nrtq := NRoot_nonneg q n zleq lt0n.
Definition zlt_nrtc : Zero[<]nrtc :=
  NRoot_pos c (less_leEq _ _ _ zltc) n lt0n zltc.

Lemma nrt_pow :
 forall (i : nat) (H : Zero[<=]q[^]i[*]c), NRoot H lt0n[=]nrtq[^]i[*]nrtc.
intros.
...
Lemma abs_pow_ltRe :
 forall s : nat -> CC,
 (forall i : nat, AbsCC (s (S i)[-]s i)[^]n[<=]q[^]i[*]c) ->
 forall i : nat, AbsIR (Re (s (S i))[-]Re (s i))[<=]nrtq[^]i[*]nrtc.
Proof.
intros s H i.
...
Lemma abs_pow_ltIm :
 forall s : nat -> CC,
 (forall i : nat, AbsCC (s (S i)[-]s i)[^]n[<=]q[^]i[*]c) ->
 forall i : nat, AbsIR (Im (s (S i))[-]Im (s i))[<=]nrtq[^]i[*]nrtc.
Proof.
intros s H i.
...
intro Hsum.
generalize (ABSIR_well_def _ _ Hsum). (* Use AbsIR_wd *)
intro Habseq.
apply
 leEq_wdl
  with (ABSIR (Sum N (pred m) (fun i : nat => Re (s (S i))[-]Re (s i)))).
2: apply eq_symmetric_unfolded; apply Habseq.
cut (N <= S (pred m)). intro leNm'.
(* FIXME was 2:Omega *) 2: clear nrtqmonz nrtqlt1 nrtc nrtq; omega.
generalize
 (triangle_SumIR N (pred m) (fun i : nat => Re (s (S i))[-]Re (s i)) leNm').
intro Htri.
apply
 leEq_transitive
  with
...
exact Htri.
generalize
 (Sum_pres_leEq (fun i : nat => AbsIR (Re (s (S i))[-]Re (s i)))
    (fun i : nat => nrtq[^]i[*]nrtc) (abs_pow_ltRe s Hi) N (
    pred m)).
intro Hlt.
...
apply
 leEq_wdl
  with (ABSIR (Sum N (pred m) (fun i : nat => Im (s (S i))[-]Im (s i)))).
2: apply eq_symmetric_unfolded; apply Habseq.
cut (N <= S (pred m)). intro leNm'.
(* FIXME was 2:Omega *) 2: clear nrtqmonz nrtqlt1 nrtc nrtq; omega.
generalize
 (triangle_SumIR N (pred m) (fun i : nat => Im (s (S i))[-]Im (s i)) leNm').
intro Htri.
apply
 leEq_transitive
  with
...
exact Htri.
generalize
 (Sum_pres_leEq (fun i : nat => AbsIR (Im (s (S i))[-]Im (s i)))
    (fun i : nat => nrtq[^]i[*]nrtc) (abs_pow_ltIm s Hi) N (
    pred m)).
intro Hlt.
apply leEq_transitive with (Sum N (pred m) (fun i : nat => nrtq[^]i[*]nrtc)).
cut (N <= pred m).
...
Lemma seq_is_CC_Cauchy :
 forall s : nat -> CC,
 (forall i : nat, AbsCC (s (S i)[-]s i)[^]n[<=]q[^]i[*]c) -> CC_Cauchy_prop s.
Proof.
...
2: apply H3.
RStepr ((nrtq[^]m[-]nrtq[^]N[/] nrtq[-]One[//]nrtqmonz)[*]nrtc).
exact H2.
...
Lemma zero_lt_posplus1 : forall x : IR, Zero[<=]x -> Zero[<]x[+]One.
Proof.
intros x zltx.
apply leEq_less_trans with x.
assumption.
exact (less_plusOne _ x).
Qed.

Lemma FTA_monic :
 forall (p : cpoly CC) (n : nat),
...
intros s Hs.
cut
 (forall i : nat, AbsCC (s (S i)[-]s i)[^]n[<=]q[^]i[*](AbsCC p ! Zero[+]One)).
intro Hs2.
...
exact
 (poly_pres_lim (fun x : CC => p ! x) (contin_polyCC p)
    (Build_CC_CauchySeq s Hs3)).
generalize
 (seq_yields_zero q qnonneg qlt10 (AbsCC p ! Zero[+]One) Hp
    (fun n0 : nat => p ! (s n0))).
intro H0. apply H0.
intro i. generalize (Hs i).
intro H1; inversion_clear H1; assumption.
exact
 (seq_is_CC_Cauchy n H0n q qnonneg qlt10 (AbsCC p ! Zero[+]One) Hp s Hs2).
