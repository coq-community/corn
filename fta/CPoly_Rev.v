...
apply
 nth_coeff_sum with (p_ := fun i0 : nat => monom (nth_coeff i0 p) (n - i0)).
apply
 eq_transitive_unfolded
  with (nth_coeff i (monom (nth_coeff (n - i) p) (n - (n - i)))).
apply
 Sum_term
  with
    (i := n - i)
...
apply eq_symmetric_unfolded.
apply
 mult_distr_sum_lft with (f := fun i1 : nat => monom (nth_coeff i1 p1) i1).
apply Sum_wd'. auto with arith. intro i1. intros.
