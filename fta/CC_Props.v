...
Record CC_CauchySeq : Type := 
  {CC_seq :> nat -> CC; CC_proof : CC_Cauchy_prop CC_seq}.
...
Definition LimCC (s : CC_CauchySeq) : CC :=
  Build_CC_set (Lim (CC_Cauchy2re s)) (Lim (CC_Cauchy2im s)).

Definition CC_SeqLimit (seq : nat -> CC) (lim : CC) : CProp :=
  forall e : IR,
  Zero[<]e -> {N : nat | forall m : nat, N <= m -> AbsCC (seq m[-]lim)[<=]e}.
...
Lemma CC_SeqLimit_uniq :
 forall (s : nat -> CC) (l l' : CC),
...
Lemma CC_SeqLimit_unq :
 forall (s : CC_CauchySeq) (l : CC), CC_SeqLimit s l -> l[=]LimCC s.
intros.
...
Definition CCfunLim (p l : CC) : CProp :=
  forall e : IR,
  Zero[<]e ->
  {d : IR | Zero[<]d |
  forall x : CC, AbsCC (p[-]x)[<=]d -> AbsCC (l[-]f x)[<=]e}.

Definition CCcontinAt (p : CC) : CProp := CCfunLim p (f p).

Definition CCcontin : CProp := forall x : CC, CCcontinAt x.

Lemma CCfunLim_SeqLimit :
 forall (p l : CC) (fl : CCfunLim p l) (pn : nat -> CC)
   (sl : CC_SeqLimit pn p), CC_SeqLimit (fun n : nat => f (pn n)) l.
Proof.
intros; unfold CC_SeqLimit in |- *.
intros eps epos.
...
Lemma poly_pres_lim :
 CCcontin ->
 forall s : CC_CauchySeq, CC_SeqLimit (fun n : nat => f (s n)) (f (LimCC s)).
Proof.
intros cp s.
...
Lemma seq_yields_zero :
 ...
 Zero[<]c ->
 forall s : nat -> CC,
 (forall i : nat, AbsCC (s i)[<=]q[^]i[*]c) -> CC_SeqLimit s Zero.
Proof.
