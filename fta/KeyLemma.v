...
This lemma is important for the Fundamental Theorem of Algebra.
*)
...
(**
%\begin{convention}%
Let [a:nat->IR] and [n:nat] such that [(lt (0) n)], [(k:nat)(Zero [<=] (a k))],
[(a n) [=] One] and [a_0 : IR], and [eps:IR] such that [(Zero [<] eps)]
and [(eps [<=] a_0)].
%\end{convention}%
*)
Variable a : nat -> IR.
...
Lemma lem_1 :
 ...
 k <= n /\
 a k[*]t[^]k[=]a_0[-]eps /\
 (forall i : nat, 1 <= i -> i <= n -> a i[*]t[^]i[<=]a_0)}}.
Proof.
cut
 (forall j : nat,
  let l := n - j in
...
clear H10 H8 H6 H5.
elim
 (cotrans_less_unfolded _ _ _ a_0_eps_fuzz (a (n - S j)[*]t'[^](n - S j)));
 intro H14.
...
Lemma chfun_1 :
 forall (k : nat -> nat) (a j i : nat), i <= j -> k i = chfun k a j i.
intros.
unfold chfun in |- *.
elim (le_gt_dec i j).
auto.
...
Lemma chfun_2 :
 forall (k : nat -> nat) (j a i : nat), j < i -> a = chfun k a j i.
intros.
unfold chfun in |- *.
elim (le_gt_dec i j).
intro y.
...
Lemma chfun_3 :
 forall (k : nat -> nat) (j a : nat),
 (forall i : nat, 1 <= k i /\ k i <= n) ->
 1 <= a -> a <= n -> forall i : nat, 1 <= chfun k a j i /\ chfun k a j i <= n.
intros.
unfold chfun in |- *.
elim (le_gt_dec i j).
auto.
...
Lemma chfun_4 :
 ...
 a <= k j -> forall i : nat, chfun k a j (S i) <= chfun k a j i.
intros.
unfold chfun in |- *.
...
Lemma Halfeps_trans :
 forall x y z : IR, x[-]Halfeps[<=]y -> y[-]Halfeps[<=]z -> x[-]eps[<=]z.
intros.
AStepl (x[-]Halfeps[-]Halfeps).
...
Lemma Key_2 :
 forall (i k k' j : nat) (ai ak ak' t : IR),
 1 <= k ->
 k < i ->
 Zero[<=]ai ->
 Zero[<=]t ->
...
Lemma Key :
 ...
 (forall j : nat,
  ...
  let k_j := k j in
  let r := t[*]p3m j in
  forall i : nat, 1 <= i -> i <= n -> a i[*]r[^]i[-]eps[<=]a k_j[*]r[^]k_j)}}.
Proof.
...
cut (0 < k' J). intro H16.
 2: elim (H10 J); auto.
elim
 (maj_upto_eps IR (fun i : nat => a i[*](t[*]p3m (S J))[^]i) (
