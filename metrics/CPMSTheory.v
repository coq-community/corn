...
set
 (H2 :=
  OneR[/] e[//]ap_symmetric_unfolded IR Zero e (less_imp_ap IR Zero e H1))
 in *. 
unfold AbsSmall in |- *.
...
apply
 shift_leEq_mult'
  with (ap_symmetric_unfolded IR Zero e (less_imp_ap IR Zero e H1)).
exact H1.
...
eapply
 existT
        with
        (P := 
          fun y0 : SubPsMetricSpace P =>
          {z : IR |
          set_glb_IR
            (fun v : IR =>
             {y1 : SubPsMetricSpace P |
             x[-d]from_SubPsMetricSpace X P y1[=]v}) z |
          x[-d]from_SubPsMetricSpace X P y0[<=]z[+]one_div_succ n})
       (x := x2).
...
eapply
 exist2T
        ...
        (Q := 
          fun z : IR =>
          x[-d]from_SubPsMetricSpace X P x2[<=]z[+]one_div_succ n)
        (x := x0).
unfold set_glb_IR in |- *.
apply H3.
...
intros xn Pn H4.
apply
 leEq_transitive with ((xn[-d]xj)[+](xj[-d]from_SubPsMetricSpace X P x0)).
case x0.
intros.
simpl in |- *.
apply ax_d_tri_ineq.
