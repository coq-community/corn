...
Proof.
 ...
 unfold Qplus in |- *.
 simpl in |- *.
 exact (a_very_specific_lemma5 (Qnum x) (Qnum y) (Qnum z) ( Qden x) (Qden y) (Qden z)).
Qed.
...
Proof.
 ...
 red in |- *.
 simpl in |- *.
 change (((Qnum x * Qden y + Qnum y * Qden x) * (Qden y * Qden x))%Z =
   ((Qnum y * Qden x + Qnum x * Qden y) * (Qden x * Qden y))%Z) in |- *.
 ring.
Qed.
...
Proof.
 ...
 red in |- *.
 simpl in |- *.
 change ((Qnum x * (Qnum y * Qden z + Qnum z * Qden y) * (Qden x * Qden y * (Qden x * Qden z)))%Z =
   ((Qnum x * Qnum y * (Qden x * Qden z) + Qnum x * Qnum z * (Qden x * Qden y)) *
...
Proof.
 ...
 red in |- *.
 apply toCProp_Zlt.
 apply a_very_specific_lemma5' with (a := Qnum x) (b := Qnum y) (c := Qnum z) (m := Qden x)
   (n := Qden y) (p := Qden z).
...
Proof.
 ...
 apply a_very_specific_lemma5' with (a := (- Qnum y)%Z) (b := (- Qnum x)%Z) (c := (- Qnum z)%Z)
   (m := Qden y) (n := Qden x) (p := Qden z).
  apply Zgt_lt.
...
Proof.
 ...
 case (Qlt_eq_gt_dec x2 y2).
  ...
  Intro J1.
  Elim (Zlt_not_eq `(Qnum x1)*((Qden y1))` `(Qnum y1)*((Qden x1))` J1).
  Symmetry.
  Assumption.
  Step_final x2.*)
  intro n.
...
Proof.
 ...
 unfold Qnum at 3 in |- *.
 unfold Qden at 7 in |- *.
 change ((Qnum x * Qden z + Qnum z * Qden x) * (Qden y * Qden z) <
