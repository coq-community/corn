...
red in |- *. intros x.
assert
 (forall (n : nat) (a b : positive), nat_of_P b < n -> Acc pp_lt (a, b)).
 simple induction n.
 intros a b H0.
 elim (lt_n_O _ H0).
...
apply
 (well_founded_ind pp_lt_wf
    (fun y : pp =>
     let (u, v) := y in
     Zdivides (Zpos (p_gcd u v)) (Zpos u) /\
     Zdivides (Zpos (p_gcd u v)) (Zpos v))).
...
intros a b.
case a; case b; intros; simpl in |- *; repeat rewrite Zopp_involutive;
 reflexivity.
Qed.
...
elim (Zgcd_duv (Zpos pb) (Zpos pa mod Zpos pb)); intros d uv; elim uv;
 intros u v.
rewrite Zmult_1_l.
reflexivity.
intro pa.
...
intros pb HB'. 
 ...
 elim (Zgcd_duv (Zpos pb) (Zabs a mod Zpos pb)); intros d uv; elim uv;
  intros u v.
 rewrite Zmult_1_l.
 reflexivity.
intros pb HB'. 
...
intros a b.
case a; case b; simpl in |- *; intros; unfold Zgcd, Zgcd_duv in |- *;
 try rewrite p_gcd_symm; auto.
Qed.
...
intros a b.
case a; case b; unfold Zgcd in |- *; simpl in |- *; intros; try discriminate;
 try tauto.
Qed.
...
intros a b q Hdiv; apply (Zdivides_trans (Zgcd a b) a q);
 [ apply Zgcd_is_divisor_lft | assumption ].
Qed.

Lemma Zdiv_gcd_elim_rht :
 forall a b q : Z, Zdivides b q -> Zdivides (Zgcd a b) q.
intros a b q Hdiv; apply (Zdivides_trans (Zgcd a b) b q);
 [ apply Zgcd_is_divisor_rht | assumption ].
Qed.
...
intros a b; rewrite (Zgcd_symm a b); rewrite (Zgcd_symm (Zgcd b a) b);
 apply Zgcd_gcd_rl.
Qed.
...
cut
 (1%Z =
  (Zgcd_coeff_a a b * (a / Zgcd a b) + Zgcd_coeff_b a b * (b / Zgcd a b))%Z).
intro H1; rewrite H1.
apply Zdivides_plus_elim.
apply Zdivides_mult_elim_lft.
apply Zgcd_is_divisor_lft.
...
generalize (Zgcd_nonzero (a * x) (b * y)); rewrite H1; intro H0; elim H0;
 auto with zarith.
intros q Hq.
rewrite <- H1.
...
case (Zdec a).
intro Ha; rewrite Ha in Hab; unfold Zrelprime in Hab;
 rewrite Zgcd_zero_lft in Hab.
 exists (Zsgn b * x)%Z; rewrite Zmult_comm; rewrite Zmult_assoc;
  rewrite (Zmult_comm b (Zsgn b)); rewrite <- Zmult_sgn_eq_abs; 
  rewrite Hab; apply Zmult_1_l.
intro Ha.
...
intros Hab0 Hrelprime.
 ...
 apply Zmult_pos_mon_lt_lft.
  apply Zlt_gt; apply Zgcd_pos; left; rewrite Zmult_comm in Hab0;
   auto with zarith.
...
repeat rewrite Zmult_plus_distr_r; repeat rewrite Zmult_plus_distr_l;
 ...
 apply Zdivides_plus_elim; apply Zdivides_plus_elim; 
 auto with zarith.
