...
apply AbsIR_wd.
apply cg_minus_wd; unfold seq_part_sum in |- *; apply Sum0_wd; intros;
 rational.
Qed.
 ...
 Build_PartFunct
  with
    (pfpfun := fun (x : IR) (Hx : I x) =>
               series_sum _
                 (fun_series_conv_imp_conv H x Hx
                    (fun n : nat => incf n x Hx))).
...
apply
 Cauchy_fun_seq_wd
  with (fun_seq_part_sum f) (fun_seq_part_sum_cont _ _ _ _ contF).
2: assumption.
intros.
apply eq_imp_Feq.
...
cut
 (forall n : nat,
  Continuous_I Hab (fun_seq_part_sum f n{+}fun_seq_part_sum g n));
 [ intro H0 | Contin ].
apply
 Cauchy_fun_seq_wd
  with
    (f := fun n : nat => fun_seq_part_sum f n{+}fun_seq_part_sum g n)
    (contf := H0).
    ...
    series_sum_plus
     with
       (x := fun n : nat =>
             Part _ _ (contin_imp_inc _ _ _ _ (contF n) x (ProjIR1 Hx')))
       (y := fun n : nat =>
             Part _ _ (contin_imp_inc _ _ _ _ (contG n) x (ProjIR2 Hx')))
       (H := H1).
...
simpl in |- *.
unfold seq_part_sum in |- *; apply cg_minus_wd; apply Sum0_wd; intros;
 rational.
Qed.
...
set (H' := fun n : nat => Continuous_I_mult _ _ _ _ _ contH (contF n)) in *;
 exists H'.
cut (forall n : nat, Continuous_I Hab (fun_seq_part_sum f n));
 [ intro H0 | Contin ].
cut (forall n : nat, Continuous_I Hab (H{*}fun_seq_part_sum f n));
 [ intro H1 | Contin ].
unfold I in |- *;
 apply Cauchy_fun_seq_wd with (fun n : nat => H{*}fun_seq_part_sum f n) H1.
2: apply
    fun_Cauchy_prop_mult
     with
       (f := fun n : nat => H)
       (contf := fun n : nat => contH)
       (g := fun_seq_part_sum f)
       (contg := H0).
intro; FEQ.
apply contin_imp_inc; Contin.
simpl in |- *.
...
apply
 fun_series_conv_imp_conv
  with
    (Hab := Hab)
    (Hx := fun n : nat => contin_imp_inc _ _ _ _ (ProjT1 convF n) x Hx');
 assumption.
Qed.
...
apply
 conv_fun_seq'_wdr
  with
    (contf := fun_seq_part_sum_cont _ _ _ _ x)
...
apply
 conv_fun_seq'_wdr
  with
    (fun_seq_part_sum_cont _ _ _ _ (ProjT1 convF))
    (Cauchy_fun_seq_Lim _ _ _ _ _ (ProjT2 convF))
    (Cauchy_cont_Lim _ _ _ _ _ (ProjT2 convF)).
...
assert
 (H' :=
  fun n : nat =>
  contin_imp_inc _ _ _ _ (fun_seq_part_sum_cont _ _ _ _ contG n)).
apply
 leEq_transitive
  with
...
apply
 leEq_transitive
  with (Part (FSum N (pred m) (fun n : nat => FRestr (H4 n))) x H5).
cut
 (Dom
    (FSum N (pred m) (fun n : nat => FRestr (contin_imp_inc _ _ _ _ (H0 n))))
    x). intro H6.
apply
 leEq_wdl
  with
...
apply le_lt_trans with k; [ idtac | apply lt_le_trans with N ];
 auto with arith.
FEQ.
unfold I in |- *; apply contin_imp_inc; Contin.
simpl in |- *.
