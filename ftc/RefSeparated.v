...
2: apply H2.
apply less_leEq;
 apply less_transitive_unfolded with (R j Hj[+]delta [/]TwoNZ).
...
elimtype False; apply lt_n_O with j'; red in |- *; apply le_trans with j;
 auto.
...
cut
 (forall Hi' : i <= n,
  delta [/]FourNZ[<]AbsIR (P i Hi'[-]R j Hj)
...
apply
 leEq_transitive
  with
    (Sumx
       (fun (i : nat) (Hi : i < n) =>
        AbsIR (F (g i Hi) just1[-]F (sep__sep_points i Hi) just2)[*]
        (P _ Hi[-]P _ (lt_le_weak _ _ Hi)))[+]
     Sumx
       (fun (i : nat) (Hi : i < n) =>
        AbsIR (F (sep__sep_points i Hi) just2)[*]
 ...
 leEq_transitive
  with
    (AbsIR
       (Sumx
          (fun (i : nat) (Hi : i < n) =>
           F (g i Hi) just1[*](P _ Hi[-]P _ (lt_le_weak _ _ Hi))[-]
           F (sep__sep_points i Hi) just2[*]
           (P _ Hi[-]P _ (lt_le_weak _ _ Hi))))[+]
     AbsIR
       (Sumx
          (fun (i : nat) (Hi : i < n) =>
           F (sep__sep_points i Hi) just2[*]
           (sep__sep_fun _ Hi[-]P _ Hi[+]
...
apply
 eq_transitive_unfolded
  with
    (AbsIR (F (g i H) just1[-]F (sep__sep_points i H) just2)[*]
