  ...
  with
    (AbsIR
       (Partition_Sum HfP incF[-]
        Partition_Sum
          (sep__part_pts_in_Partition _ _ _ F contF Hab' _ P _ RL_alpha _ RL_csi1
             RL_delta1 _ HfP) incF)).
apply sep__part_Sum.
assumption.
  ...
  with
    (AbsIR
       (Partition_Sum HfR incF[-]
        Partition_Sum
          (sep__part_pts_in_Partition _ _ _ F contF Hab' _ R _ RL_alpha _ RL_csi2
             RL_delta2 _ HfR) incF)).
apply sep__part_Sum.
assumption.
apply AbsIR_wd; apply cg_minus_wd.
algebra.
...
apply
 leEq_wdl
  with
    (AbsIR
       (Partition_Sum RL_fP'_in_P' incF[-]
        Partition_Sum
...
apply
 leEq_wdl
  with
    (AbsIR
       (Partition_Sum HfP incF[-]Partition_Sum RL_fP'_in_P' incF[+]
...
eapply
 second_refinement_lemma
                         with
                         (Q := Separated_Refinement _ _ _ _ _ _ _ RL_Q_sep)
                        (He := He)
                        (He' := He').
