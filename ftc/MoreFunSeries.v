...
intros.
apply
 conv_Cauchy_fun_seq' with F (included_imp_Continuous _ _ contF _ _ _ Hinc);
 auto.
Qed.
...
set
 (contf' :=
  fun i : nat =>
  included_imp_Continuous J (f i) (contf i) _ _ (leEq_reflexive _ x) H0) 
 in *.
apply
 Cauchy_prop_wd
  with
    (fun n : nat =>
     Part (f n) x
       ((fun i : nat =>
         contin_imp_inc _ _ (leEq_reflexive _ x) (f i) (contf' i)) n x
...
apply
 FLim_unique
  with
    f
    (fun n : nat => included_imp_Continuous _ _ (contf n) _ _ _ H1)
    (included_imp_Continuous _ _ HF _ _ _ H1)
...
cut (Continuous_I Hab (Cauchy_fun_seq_Lim _ _ _ _ _ (Hf a b Hab Hinc)));
 [ intro H0 | Contin ].
cut (Continuous_I Hab (Cauchy_fun_seq_Lim _ _ _ _ _ (Hg a b Hab Hinc)));
 [ intro H1 | Contin ].
...
cut
 (Continuous J
    (Cauchy_fun_seq_Lim_IR _ _ _ Hf{+}Cauchy_fun_seq_Lim_IR _ _ _ Hg));
 [ intro H0 | Contin ].
apply
 conv_Cauchy_fun_seq'_IR
...
cut (m = S (pred m));
 [ intro
 | apply S_pred with 0; apply lt_le_trans with (S N); auto with arith ].
cut (n = S (pred n));
 [ intro
 | apply S_pred with 0; apply lt_le_trans with (S N); auto with arith ].
...
assert
 (convS :=
  FSeries_conv _ _ insert_series_conv
    (Continuous_Sum0 _ _ insert_series_cont)
...
eapply eq_transitive_unfolded.
2: apply
    (insert_series_sum_char n x
       (contin_imp_inc _ _ _ _
          (included_imp_Continuous _ _ (Continuous_Sum0 _ _ contF n) _ _ _
             Hinc) _ Hx)
       (contin_imp_inc _ _ _ _
