...
Lemma Continuous_imp_comm_Lim :
 forall F x e,
 Zero[<]e ->
 Continuous (clcr (Lim x[-]e) (Lim x[+]e)) F ->
 forall Hx Hxn H,
 F (Lim x) Hx[=]Lim (Build_CauchySeq IR (fun n : nat => F (x n) (Hxn n)) H).
...
(**
This is a tricky result: if [F] is continuous and positive in both $[a,b]$#[a,b]# and $]b,c]$#]b,c]#, then it is positive in $[a,c]$#[a,c]#.
*)

Lemma Continuous_imp_pos :
 forall (a b c : IR) (Hac : a[<=]c),
 a[<=]b ->
 b[<]c ->
 forall F : PartIR,
 Continuous_I Hac F ->
 (forall t : IR, a[<=]t -> t[<=]b -> forall Ht, Zero[<]F t Ht) ->
 (forall t : IR, b[<]t -> t[<=]c -> forall Ht, Zero[<]F t Ht) ->
 forall t : IR, a[<=]t -> t[<=]c -> forall Ht, Zero[<]F t Ht.
...
2: AStepl ZeroR; auto.
intro H11.
elim (cotrans_less_unfolded _ _ _ H11 t); intro.
clear H11.
elim (cotrans_less_unfolded _ _ _ H9 (t[-]b)); intro.
apply H3.
...
Lemma strict_inc_glues :
 forall (a b c : IR) (F : PartIR) (Hab : a[<=]b) (Hbc : b[<=]c)
   (Hac : a[<=]c),
 included (Compact Hac) (Dom F) ->
 (forall x y : IR,
  Compact Hab x -> Compact Hab y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy) ->
 (forall x y : IR,
  Compact Hbc x -> Compact Hbc y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy) ->
 forall x y : IR,
 Compact Hac x -> Compact Hac y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy.
...
elim (cotrans_less_unfolded _ _ _ H4 b); intro.
...
Lemma strict_inc_glues' :
 forall (a b c : IR) (F : PartIR),
 a[<]b ->
 b[<]c ->
 included (olor a c) (Dom F) ->
 (forall x y : IR,
  olcr a b x -> olcr a b y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy) ->
 (forall x y : IR,
  clor b c x -> clor b c y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy) ->
 forall x y : IR,
 olor a c x -> olor a c y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy.
...
Lemma olor_pos_clor_nonneg :
 forall (a b : IR) (F : PartIR),
 (forall x : IR, olor a b x -> forall Hx, Zero[<]F x Hx) ->
 forall Ha,
 Zero[<=]F a Ha -> forall x : IR, clor a b x -> forall Hx, Zero[<=]F x Hx.
...
Lemma olor_pos_olcr_nonneg :
 forall (a b : IR) (F : PartIR),
 (forall x : IR, olor a b x -> forall Hx, Zero[<]F x Hx) ->
 forall Hb,
 Zero[<=]F b Hb -> forall x : IR, olcr a b x -> forall Hx, Zero[<=]F x Hx.
intros a b F H Ha H0 x H1 Hx H2.
cut (Not (olor a b x)); intro H3.
cut (x[=]b). intro H4.
apply H0.
...
Lemma olor_pos_clcr_nonneg :
 ...
 Zero[<=]F a Ha ->
 forall Hb,
 Zero[<=]F b Hb -> forall x : IR, clcr a b x -> forall Hx, Zero[<=]F x Hx.
intros a b F Hab H Ha H0 Hb H1 x H2 Hx H3.
cut (Not (olor a b x)); intro H4.
elim (cotrans_less_unfolded _ _ _ Hab x); intro H5.
cut (x[=]b). intro H6.
apply H1.
...
Lemma FConst_prop :
 forall J pJ (F' : PartIR),
 Derivative J pJ F' [-C-]Zero -> {c : IR | Feq J F' [-C-]c}.
intros J pJ F' H.
...
Lemma Feq_crit_with_const :
 forall J pJ F G H,
 Derivative J pJ F H ->
 Derivative J pJ G H -> {c : IR | Feq J (F{-}G) [-C-]c}.
intros.
...
(**
This yields the following known result: any differential equation of the form $f'=g$#f'=g# with initial condition [f(a) [=] b] has a unique solution.
*)
...
Lemma Feq_criterium :
 ...
 Derivative J pJ F H ->
 Derivative J pJ G H ->
 forall x : IR, J x -> (forall Hx Hx', F x Hx[=]G x Hx') -> Feq J F G.
...
Lemma Derivative_imp_resp_less :
 forall J pJ a b F F',
 Derivative J pJ F F' ->
 a[<]b ->
 J a ->
 J b ->
 (forall contF', Zero[<]glb_funct _ _ (Min_leEq_Max a b) F' contF') ->
...
Lemma Derivative_imp_resp_leEq :
 forall J pJ a b F F',
 Derivative J pJ F F' ->
 a[<=]b ->
 J a ->
 J b ->
 (forall contF', Zero[<=]glb_funct _ _ (Min_leEq_Max b a) F' contF') ->
 forall Ha Hb, F a Ha[<=]F b Hb.
intros J pJ a b F F' derF Hab HaJ HbJ Hglb Ha Hb.
...
Lemma nexp_resp_leEq_odd :
 forall n : nat, odd n -> forall x y : IR, x[<=]y -> x[^]n[<=]y[^]n.
intro; case n.
