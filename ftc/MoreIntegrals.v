...
%\begin{convention}% Let [a,b:IR] and assume [F,G] are partial functions continuous in $[\min(a,b),\max(a,b)]$#[min(a,b),max(a,b)]#.
%\end{convention}%
...
Lemma compact_inc_Min_lft :
 forall H, included (compact (Min a b) a H) (Compact Hab).
intros.
...
Lemma compact_inc_Min_rht :
 forall H, included (compact (Min a b) b H) (Compact Hab).
intros.
apply included_compact; split.
apply leEq_reflexive.
apply Min_leEq_Max.
...
(**
The integral is defined by the formula $\int_a^bf=\int_{\min(a,b)}^bf-\int_{\min(a,b)}^af$#&int;<sub>a</sub><sup>b</sup>f=&int;<sub>min(a,b)</sub><sup>b</sup>f-&int;<sub>min(a,b)</sub><sup>a</sup>f#, inspired by the domain union rule; obviously it coincides with the classical definition, and it collapses to the old one in the case [a [<=] b].
*)
...
Definition Integral :=
  integral _ _ (Min_leEq_rht a b) F Integral_inc2[-]
  integral _ _ (Min_leEq_lft a b) _ Integral_inc1.

Lemma Integral_integral :
 forall (Hab' : a[<=]b) HF', Integral[=]integral _ _ Hab' F HF'.
intros.
unfold Integral in |- *.
...
All our old properties carry over to this new definition---and some new ones, too.  We begin with extensionality and strong extensionality.
*)
...
Lemma Integral_strext' :
 ...
 Integral (Hab:=Hab) (F:=F) HF1[#]
 Integral (a:=c) (b:=d) (Hab:=Hcd) (F:=F) HF2 -> a[#]c or b[#]d.
...
Lemma Integral_comm_scal :
 forall (c : IR) (H : Continuous_I Hab (c{**}F)),
...
Lemma linear_Integral :
 forall (alpha beta : IR) (H : Continuous_I Hab (alpha{**}F{+}beta{**}G)),
...
Lemma integral_plus_Integral :
 forall a b Hab F c Hac Hcb Hab' Hac' Hcb',
...
(**
Notice that, unlike in the classical case, an extra hypothesis (the continuity of [F] in the interval $[\min(a,b,c),\max(a,b,c)]$#[min(a,b,c),max(a,b,c)]#) must be assumed.
*)
...
Lemma Integral_op :
 forall Hab' (contF' : Continuous_I (a:=Min b a) (b:=Max b a) Hab' F),
...
Lemma ub_Integral :
 a[#]b ->
 forall c : IR,
 (forall x : IR, Compact Hab x -> forall Hx, AbsIR (F x Hx)[<=]c) ->
 forall x : IR,
 Compact Hab x ->
 forall Hx, AbsIR (F x Hx)[<]c -> AbsIR (Integral contF)[<]c[*]AbsIR (b[-]a).
intros H c H0 x H1 Hx H2.
set (N := Norm_Funct contF) in *.
cut (N[<=]c); intros.
elim (cotrans_less_unfolded _ _ _ H2 N); intros.
...
Lemma Integral_eq_zero :
 forall a b Hab (F : PartIR) contF (x : IR),
 Compact Hab x ->
 (forall Hx, Zero[<]F x Hx) ->
 (forall x : IR, Compact Hab x -> forall Hx, Zero[<=]F x Hx) ->
