...
Proof.
 ...
 cut (forall x y : subset I, scs_elem _ _ x [=] scs_elem _ _ y -> fiSp1 x [=] fiSp2 y); intros.
  ...
  Lazy_Eq.
   ...
   simpl in |- *; algebra.
  unfold pp in |- *.
  rstepr (nring (S p) [*] (One[/] _[//] mult_resp_ap_zero _ _ _ (nring_fac_ap_zero _ p)
...
Proof.
 intro; induction  n as [| n Hrecn].
  ...
  apply Derivative_I_wdl with (funct_i' 0 Hf 0 (lt_n_Sn 0)).
   ...
   intros; simpl in |- *.
   apply eq_transitive_unfolded with (Zero[+] fi 0 Hf 0 (lt_n_Sn 0)
...
Proof.
 intros.
 assert (H := TLH).
 cut {c : IR | I c | forall Hc, AbsIR (g' n Hf Hf' H c Hc) [<=] e[*]AbsIR (One[/] _[//]H)};
