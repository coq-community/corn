...
- [a,b:IR] and [I] will denote $[a,b]$#[a,b]#;
- [c,d:IR] and [J] will denote $[a,b]$#[a,b]#;
...
Definition maps_into_compacts :=
  c[<]d
  and included (Compact Hcd) (Dom G)
      and (forall (x : IR) (Hx : Dom F x), I x -> Compact Hcd (F x Hx)).
...
Lemma maps_lemma' :
 forall (x : IR) (Hx : Dom F x), I x -> Compact Hcd (F x Hx).
...
Lemma maps_lemma :
 forall x : IR, I x -> forall Hx : Dom F x, Compact Hcd (F x Hx).
intros.
simpl in |- *.
apply maps_lemma'.
assumption.
...
exact
 (included_FMult _ _ _ (included_comp _ _ _ _ _ _ _ _ incF incG' maps') incF').
intros e He.
...
exists
 (IntPartIR
    (included_FMult _ _ _ (included_comp _ _ _ _ _ _ _ _ incF incG' maps')
       incF')).
...
Definition maps_compacts_into (F : PartIR) :=
  forall a b Hab,
  included (compact a b Hab) I ->
  {c : IR |
  {d : IR |
  {Hcd : _ |
  included (compact c d (less_leEq _ _ _ Hcd)) J
  and (forall (x : IR) Hx,
       Compact Hab x -> Compact (less_leEq _ _ _ Hcd) (F x Hx))}}}.

(**
Now everything comes naturally:
*)

Lemma comp_inc_lemma :
 forall F, maps_compacts_into F -> forall (x : IR) Hx, I x -> J (F x Hx).
intros F H x Hx H0.
...
Lemma Continuous_comp :
 Continuous I F -> Continuous J G -> Continuous I (G[o]F).
intros H H0.
...
Lemma Derivative_comp :
 Derivative I pI F F' ->
 Derivative J pJ G G' -> Derivative I pI (G[o]F) ((G'[o]F){*}F').
intros H H0.
...
Definition positive_fun (P : IR -> CProp) (F : PartIR) :=
  included P (Dom F)
  and {c : IR | Zero[<]c | forall y : IR, P y -> forall Hy, c[<=]F y Hy}.

Definition negative_fun (P : IR -> CProp) (F : PartIR) :=
  included P (Dom F)
  and {c : IR | c[<]Zero | forall y : IR, P y -> forall Hy, F y Hy[<=]c}.

Lemma positive_imp_maps_compacts_into :
 forall (J : interval) F,
...
Lemma negative_imp_maps_compacts_into :
 forall (J : interval) F,
...
Lemma Continuous_imp_maps_compacts_into :
 forall (J : interval) F, Continuous J F -> maps_compacts_into J realline F.
...
Lemma Diffble_comp :
 forall I J pI pJ F G,
 maps_compacts_into I J F ->
