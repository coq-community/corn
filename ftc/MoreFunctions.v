...
apply
 included_imp_diffble_n
  with
    (Hab' := proper_compact_in_interval2' _ _ _ _ _ _
...
apply
 (iprop_compact_in_interval_inc1 _ _ _ _
    (compact_compact_in_interval I pI x Hx)
...
apply Feq_symmetric;
 apply (N_Deriv_Feq n F (CAnd_intro _ _ incF diffF) _ _ Hab H1 H0).
apply n_deriv_lemma.
Qed.
...
apply Feq_symmetric;
 apply (N_Deriv_Feq n F (CAnd_intro _ _ incF diffFn) _ _ Hab H1 H0).
eapply Derivative_I_wdr.
apply Feq_symmetric;
 apply (N_Deriv_Feq _ _ (CAnd_intro _ _ incF' diffFSn) _ _ Hab H2 H0).
apply n_Sn_deriv.
Qed.
...
cut (Diffble_n n I pI F);
 [ intro H1 | eapply Derivative_n_imp_Diffble_n; apply H ].
cut (Diffble_n (S n) I pI F);
 [ intro H2 | eapply Derivative_n_imp_Diffble_n; apply H0 ].
