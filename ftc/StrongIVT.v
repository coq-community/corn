...
Lemma IVT'_seq_lemma :
 forall (xy : prodT IR IR) (x:=fstT xy) (y:=sndT xy)
   (Hxy : prodT (I x) (I y)),
 let Hx := fstT Hxy in
 let Hy := sndT Hxy in
 x[<]y ->
 F x (incF _ Hx)[<]z and z[<]F y (incF _ Hy) ->
 {xy0 : prodT IR IR |
 let x0 := fstT xy0 in
 let y0 := sndT xy0 in
 {Hxy0 : prodT (I x0) (I y0) |
...
apply less_transitive_unfolded with y1; unfold x1, y1 in |- *;
...
apply less_transitive_unfolded with x1; unfold x1, y1 in |- *;
...
elim
 (IVT'_seq_lemma (pairT IVT'seq3 IVT'seq4) (pairT IVT'H3 IVT'H4) IVT'prf0
...
Lemma a'_seq_less_b'_seq : forall n : nat, a'_seq n[<]b'_seq n.
exact (fun n : nat => IVT'prf (IVT'_seq n)).
Qed.

Lemma a'_seq_less_z : forall n : nat, F _ (incF _ (a'_seq_I n))[<]z.
exact (fun n : nat => IVT'z1 (IVT'_seq n)).
Qed.

Lemma z_less_b'_seq : forall n : nat, z[<]F _ (incF _ (b'_seq_I n)).
exact (fun n : nat => IVT'z2 (IVT'_seq n)).
Qed.
...
Lemma a'_seq_b'_seq_dist :
 forall n : nat, b'_seq n[-]a'_seq n[=](Two [/]ThreeNZ)[^]n[*](b[-]a).
simple induction n.
simpl in |- *; Algebra.
clear n; intros.
...
2: apply inv_resp_leEq;
    apply local_mon'_imp_mon' with (f := fun n : nat => [--](b'_seq n)); 
    auto.
...
Lemma Weak_IVT :
 forall I F,
 Continuous I F ->
 forall a b Ha Hb (HFab : F a Ha[<]F b Hb),
 I a ->
 I b ->
 forall e : IR,
 Zero[<]e ->
 forall y : IR,
 Compact (less_leEq _ _ _ HFab) y ->
...
Lemma IVT_inc :
 ...
 (forall x y : IR, I x -> I y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy) ->
 forall y : IR,
 Compact (Min_leEq_Max (F a Ha) (F b Hb)) y ->
...
2: apply included_imp_Continuous with I; auto; apply included_interval; auto.
cut (Min a b[=]a);
 [ intro | apply leEq_imp_Min_is_lft; apply less_leEq; auto ].
cut (Max a b[=]b);
...
Lemma IVT_dec :
 ...
 I a ->
 I b ->
 (forall x y : IR, I x -> I y -> x[<]y -> forall Hx Hy, F y Hy[<]F x Hx) ->
 forall y : IR,
 Compact (Min_leEq_Max (F a Ha) (F b Hb)) y ->
 {x : IR | Compact (Min_leEq_Max a b) x | forall Hx, F x Hx[=]y}.
intros.
...
inversion_clear X4; split; simpl in |- *.
apply inv_resp_leEq.
eapply leEq_wdr.
apply H0.
...
Lemma IVT'_inc :
 ...
 (forall x y : IR, I x -> I y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy) ->
 forall y : IR,
 olor (Min (F a Ha) (F b Hb)) (Max (F a Ha) (F b Hb)) y ->
