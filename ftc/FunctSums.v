...
We will first consider the case of a family $\{f_i\}_{i\in\NN}$#{f_i}# of functions; we can both define $\sum_{i=0}^{n-1}f_i$#the sum of the first n functions# ([FSum0]) or $\sum_{i=m}^nf_i$#the sum of f_m through f_n# ([FSum]).
*)
...
Lemma FSum0_wd :
 forall (m : nat) (f g : nat -> PartIR),
 (forall (x : IR) HxF HxG (i : nat), f i x (HxF i)[=]g i x (HxG i)) ->
 forall (x : IR) HxF HxG, FSum0 m f x HxF[=]FSum0 m g x HxG.
intros.
simpl in |- *.
apply Sum0_wd.
...
Lemma FSum_one :
 forall (n : nat) (f : nat -> PartIR) (x : IR) Hx Hx',
...
Lemma FSum_FSum :
 forall (l m n : nat) (f : nat -> PartIR) x Hx Hx' Hx'',
...
Lemma FSum_wd :
 forall (m n : nat) (f g : nat -> PartIR),
 (forall (x : IR) HxF HxG (i : nat), f i x (HxF i)[=]g i x (HxG i)) ->
 forall (x : IR) HxF HxG, FSum m n f x HxF[=]FSum m n g x HxG.
intros.
simpl in |- *.
apply Sum_wd.
Algebra.
...
Lemma FSum_plus_FSum :
 forall (f g : nat -> PartIR) (m n : nat) (x : IR) Hx HxF HxG,
 FSum m n (fun i : nat => f i{+}g i) x Hx[=]
...
Lemma inv_FSum :
 forall (f : nat -> PartIR) (m n : nat) (x : IR) Hx Hx',
 FSum m n (fun i : nat => {--}(f i)) x Hx[=][--](FSum m n f x Hx').
intros.
simpl in |- *.
eapply eq_transitive_unfolded.
2: apply inv_Sum.
...
Lemma FSum_wd' :
 ...
 (forall (x : IR) HxF HxG (i : nat),
  m <= i -> i <= n -> f i x (HxF i)[=]g i x (HxG i)) ->
 forall (x : IR) HxF HxG, FSum m n f x HxF[=]FSum m n g x HxG.
intros.
simpl in |- *.
...
Lemma FSum_resp_less :
 forall (f g : nat -> PartIR) (m n : nat),
 m <= n ->
 (forall (x : IR) HxF HxG (i : nat),
  m <= i -> i <= n -> f i x (HxF i)[<]g i x (HxG i)) ->
 forall (x : IR) HxF HxG, FSum m n f x HxF[<]FSum m n g x HxG.
...
Lemma FSum_comm_scal :
 forall (f : nat -> PartIR) c m n (x : IR) Hx Hx',
 FSum m n (fun i : nat => f i{*}[-C-]c) x Hx[=](FSum m n f{*}[-C-]c) x Hx'.
intros.
simpl in |- *.
eapply eq_transitive_unfolded.
...
Lemma FSum_comm_scal' :
 forall (f : nat -> PartIR) c m n (x : IR) Hx Hx',
 FSum m n (fun i : nat => [-C-]c{*}f i) x Hx[=]([-C-]c{*}FSum m n f) x Hx'.
intros.
simpl in |- *.
eapply eq_transitive_unfolded.
2: apply (Sum_comm_scal' c (fun n : nat => f n x (ProjIR2 Hx' n)) m n).
apply Sum_wd; intros; rational.
Qed.
...
(**
Also important is the case when we have a finite family $\{f_i\}_{i=0}^{n-1}$ of #exactly n# functions; in this case we need to use the [FSumx] operator.
*)

Fixpoint FSumx (n : nat) : (forall i : nat, i < n -> PartIR) -> PartIR :=
  match n return ((forall i : nat, i < n -> PartIR) -> PartIR) with
  | O => fun _ : forall i : nat, i < 0 -> PartIR => [-C-]Zero
  | S p =>
      fun f : forall i : nat, i < S p -> PartIR =>
      FSumx p (fun (i : nat) (l : i < p) => f i (lt_S i p l)){+}
      f p (lt_n_Sn p)
  end.
...
Lemma FSumx_wd :
 forall (n : nat) (f g : forall i : nat, i < n -> PartIR),
 (forall (i : nat) (Hi : i < n) (x : IR) HxF HxG, f i Hi x HxF[=]g i Hi x HxG) ->
 forall (x : IR) HxF HxG, FSumx _ f x HxF[=]FSumx _ g x HxG.
intro; case n.
intros; simpl in |- *; Algebra.
clear n.
simple induction n.
...
Lemma FSumx_wd' :
 forall (P : IR -> CProp) (n : nat) (f g : forall i : nat, i < n -> PartIR),
 (forall (i : nat) H H', Feq P (f i H) (g i H')) ->
 Feq P (FSumx _ f) (FSumx _ g).
...
(**
As was already the case for [Sumx], in many cases we will need to explicitly assume that $f_i$#f_1# is independent of the proof that [i [<] n].  This holds both for the value and the domain of the partial function $f_i$#f_i#.
*)

Definition good_fun_seq (n : nat) (f : forall i : nat, i < n -> PartIR) :=
  forall i j : nat,
  i = j ->
  forall (H : i < n) (H' : j < n) (x y : IR),
  x[=]y -> forall Hx Hy, f i H x Hx[=]f j H' y Hy.

Definition good_fun_seq' (n : nat) (f : forall i : nat, i < n -> PartIR) :=
  forall i j : nat,
  i = j ->
  forall (H : i < n) (H' : j < n) (x y : IR),
  x[=]y -> Dom (f i H) x -> Dom (f j H') y.
...
Lemma FSumx_pred :
 ...
 good_fun_seq' _ f ->
 forall x : IR,
 Dom (FSumx n f) x -> forall (i : nat) (Hi : i < n), Dom (f i Hi) x.
intros n f H x H0 i Hi; induction  n as [| n Hrecn].
elimtype False; inversion Hi.
simpl in H.
elim (le_lt_eq_dec _ _ Hi); intro.
cut (i < n); [ intro | auto with arith ].
set (g := fun (i : nat) (Hi : i < n) => f i (lt_S _ _ Hi)) in *.
...
Lemma FSumx_pred' :
 ...
 good_fun_seq' _ f ->
 forall x : IR,
 (forall (i : nat) (Hi : i < n), Dom (f i Hi) x) -> Dom (FSumx n f) x.
...
Lemma FSumx_char :
 forall n f x Hx Hf,
 FSumx n f x Hx[=]
 Sumx (fun (i : nat) (Hi : i < n) => f i Hi x (FSumx_pred n f Hf x Hx i Hi)).
intro; induction  n as [| n Hrecn].
Algebra.
intros; simpl in |- *.
apply bin_op_wd_unfolded; Algebra.
cut (good_fun_seq' n (fun (i : nat) Hi => f i (lt_S i n Hi))).
intro H.
eapply eq_transitive_unfolded.
...
Definition FSumx_to_FSum (n : nat) :
  (forall i : nat, i < n -> PartIR) -> nat -> PartIR.
intros n f i.
...
Lemma FSumx_lt :
 forall (n : nat) (f : forall i : nat, i < n -> PartIR),
 good_fun_seq _ f ->
 forall (i : nat) (Hi : i < n) (x : IR) Hx Hx',
...
Lemma FSumx_le :
 forall (n : nat) (f : forall i : nat, i < n -> PartIR),
 good_fun_seq _ f ->
 forall (i : nat) (Hi : n <= i) (x : IR) Hx, FSumx_to_FSum n f i x Hx[=]Zero.
do 6 intro.
unfold FSumx_to_FSum in |- *.
elim (le_lt_dec n i); intro; simpl in |- *.
intro; Algebra.
...
Lemma FSum_FSumx_to_FSum :
 forall (n : nat) (f : forall i : nat, i < S n -> PartIR),
 good_fun_seq _ f ->
 good_fun_seq' _ f ->
 forall (x : IR) Hx Hx', FSum 0 n (FSumx_to_FSum _ f) x Hx[=]FSumx _ f x Hx'.
simple induction n.
intros; simpl in |- *.
eapply eq_transitive_unfolded.
apply Sum_one.
...
apply Sum_last.
apply bin_op_wd_unfolded.
set (g := fun (i : nat) (l : i < S n) => f i (lt_S _ _ l)) in *.
cut (good_fun_seq _ g); intros.
cut (good_fun_seq' _ g).
intro H3.
AStepr
 (FSumx n (fun (i : nat) (l : i < n) => g i (lt_S _ _ l)) x
...
unfold g in |- *; red in |- *; intros.
red in H1; apply H1 with i (lt_S _ _ H4) x0; auto.
unfold g in |- *; red in |- *; intros.
...
Lemma FSum0_0 :
 forall P f,
 (forall n, included P (Dom (f n))) -> Feq P [-C-]Zero (FSum0 0 f).
intros P f H.
FEQ.
simpl in |- *.
...
Lemma FSum_0 :
 forall P f n, (forall i, included P (Dom (f i))) -> Feq P (f n) (FSum n n f).
intros P f n H.
FEQ.
simpl in |- *.
red in |- *; intros; apply (H n0); auto.
