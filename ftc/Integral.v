...
unfold l' at 1 in |- *.
cut (S (n + m) < S l');
 [ intro | unfold l' in |- *; simpl in |- *; rewrite plus_comm; auto ].
    ...
    (AbsIR
       (Lim
          (Build_CauchySeq _ _
             (Cauchy_minus
                (Cauchy_const
                   (Partition_Sum HfP (contin_imp_inc _ _ _ _ contF)))
                (Build_CauchySeq _ _ Cauchy_Darboux_Seq))))).
...
generalize (proj2b_sig2T _ _ _ (contF' _ Hee));
 generalize (proj2a_sig2T _ _ _ (contF' _ Hee)); fold d' in |- *;
 intros Hd' Hd'0.
...
inversion_clear H4.
generalize
 (H2 (contin_imp_inc _ _ _ _ contF x H1) (contin_imp_inc _ _ _ _ contG x H1)).
apply eq_imp_not_ap.
auto.
Qed.
...
apply
 Mengolli_Sum
  with
    (f := fun (i : nat) (Hi : i <= S n) => Even_Partition Hab _ (O_S n) i Hi).
red in |- *; intros.
apply prf1; auto.
...
unfold integral_seq, Even_Partition_Sum, Partition_Sum in |- *.
apply
 eq_transitive_unfolded with (Sumx (fun (i : nat) (H : i < S n) => ZeroR)).
...
unfold partition_join_fun in |- *.
elim le_lt_dec; intro; simpl in |- *.
elimtype False; apply le_Sn_n with n; apply le_trans with (S (n + m));
 auto with arith.
...
unfold partition_join_fun in |- *; elim le_lt_dec; simpl in |- *; intro;
 [ apply prf1; auto
 | elimtype False; apply le_not_lt with n i; auto with arith ].
unfold partition_join_fun in |- *; elim le_lt_dec; simpl in |- *; intro;
 [ apply prf1; auto
...
apply
 less_wdl
  with
    (AbsIR
       (integral _ _ _ _ cont1[+]integral _ _ _ _ cont2[+]
...
apply
 less_wdr
  with
    (integral _ _ _ _ cont1[+]integral _ _ _ _ cont2[+]integral _ _ _ _ cont3).
2: apply
    eq_transitive_unfolded
