...
Proof.
 intros x e.
 induction  e as [i| | x y e1 Hrece1 e0 Hrece0| x k e Hrece| x f e Hrece| x y f e1 Hrece1 e0 Hrece0| x
   f e Hrece Hx| x e Hrece| x y e1 Hrece1 e0 Hrece0].
    ...
    apply (interpG_binop (xforgetG _ e1) (xforgetG _ e0) f x y (binop f x y)); algebra.
   eapply (interpG_part (xforgetG _ e) f x (pfun f x Hx)).
    apply eq_reflexive_unfolded.
   algebra.
  apply (interpG_mult_int (xforgetG _ e) (-1) x); algebra.
 apply (interpG_plus (xforgetG _ e1) (xforgetG _ (xexprG_inv _ e0)) x [--]y (x[-]y)); algebra.
...
Proof.
 intro e.
 induction  e as [v| z| e1 Hrece1 e0 Hrece0| e1 Hrece1 e0 Hrece0| e1 Hrece1 e0 Hrece0| u e Hrece| b e1
   Hrece1 e0 Hrece0| p e Hrece].
        intros x y Hx Hy.
        inversion Hx.
        inversion Hy.
        Step_final (val v).
...
Proof.
 simple induction z; [ induction  k as [| p| p] | induction  k as [| p| p]
   | induction  k as [| p| p] ]; simpl in |- *; auto; intros; rewrite Pmult_comm; auto.
Qed.
...
Proof.
 cut (forall (i j : Z) (x y : G),
   II (expr_int i) x -> II (expr_int j) y -> II (expr_int (i + j)) (x[+]y)).
  cut (forall (e f : expr) (x y : G), II e x -> II f y -> II (expr_plus e f) (x[+]y)).
...
Proof.
 cut (forall (e1 e2 f : expr) (x y : G), (forall (f : expr) (x y : G),
   II e2 x -> II f y -> II (PM_plus e2 f) (x[+]y)) -> II (expr_plus e1 e2) x ->
  ...
  cut (forall (e1 e2 f : expr) (x y : G), (forall (f : expr) (x y : G),
   ...
   cut (forall (e f : expr) (x y : G), II e x -> II f y -> II (MM_plus e f) (x[+]y)).
    cut (forall (e f : expr) (x y : G), II e x -> II f y -> II (expr_plus e f) (x[+]y)).
     cut (forall (e f : expr) (x y : G), II e x -> II f y -> II (expr_plus f e) (x[+]y)).
...
Proof.
 intro e; induction  e as [v| z| e1 Hrece1 e0 Hrece0| e1 Hrece1 e0 Hrece0| e1 Hrece1 e0 Hrece0| u e
   Hrece| b e1 Hrece1 e0 Hrece0| p e Hrece]; simpl in |- *; auto.
      ...
      apply interpG_wd with (zmult y k[+]zmult x1 k).
       2: astepl (zmult (y[+]x1) k); astepl (zmult (x1[+]y) k); Step_final (zmult x0 k).
...
Proof.
 intro; elim e; intros; simpl in |- *.
        apply (interpG_plus G val unop binop pfun (expr_mult (expr_var v) expr_one) expr_zero x (Zero:G) x).
          algebra.
   ...
   apply (interpG_plus G val unop binop pfun (expr_mult (expr_unop u (NormG e0)) expr_one) expr_zero x (
     Zero:G) x).
     algebra.
  ...
  apply (interpG_plus G val unop binop pfun
    (expr_mult (expr_binop b (NormG e0) (NormG e1)) expr_one) expr_zero x (Zero:G) x).
    algebra.
   apply (interpG_mult_int G val unop binop pfun (expr_binop b (NormG e0) (NormG e1)) 1 x); algebra.
   apply (interpG_binop G val unop binop pfun (NormG e0) (NormG e1) b x0 y); algebra.
  apply interpG_zero; algebra.
 inversion X0.
 ...
 apply (interpG_plus G val unop binop pfun (expr_mult (expr_part p (NormG e0)) expr_one) expr_zero x (
   Zero:G) x).
   algebra.
  apply (interpG_mult_int G val unop binop pfun (expr_part p (NormG e0)) 1 x); algebra.
  apply (interpG_part G val unop binop pfun (NormG e0) p x0) with (Hx := Hx); algebra.
 apply interpG_zero; algebra.
Qed.
