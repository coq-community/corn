...
FEQ.
 ...
 apply included_FMinus;
  apply included_FComp;  Included;  repeat constructor.
simpl.
change (Half (R:=IR)[*]
...
destruct n as [|n].
 ...
 simpl; rational.
rstepl (Half (R:=IR)[*]
 (Log_series_coef (S n)[*](x[^]S n[-]([--]x)[^]S n))).
destruct (even_odd_dec (S n)) as [A|A];
 unfold cg_minus.
...
assert (F0:Dom F c).
 destruct Hc0 as [A B].
 repeat (constructor || exists (CAnd_intro _ _ CI CI));
 simpl.
  apply shift_less_plus'.
  rstepl ([--]One:IR).
  assumption.
