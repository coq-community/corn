...
elim
 (bndf r' H' _ _ (Min_leEq_Max' x y t)
    (included_interval' realline _ _ _ _ CI CI CI CI _) e He); 
 intros N HN.
...
cut
 (forall n : nat,
  Continuous realline (FSum0 n (Taylor_Series _ _ _ _ Ha _ derF))).
intro H2.
...
eapply
 FLim_unique_IR
                ...
                (HG := H0)
               (HF := H3)
               (f := fun n : nat => FSum0 n (Taylor_Series _ _ _ _ Ha _ derF))
               (contf := H2).
...
elim (Taylor_Series_conv_lemma1 a _ _ Hab _ (pos_div_two _ _ H4));
 intros N HN.
...
elim
 (Taylor' _ _ _ _ _ Ha (Hinc x Hx) n
    (Derivative_n_imp_Diffble_n _ _ _ _ _ (derF (S n)))
    (Derivative_n_imp_Diffble_n _ _ _ _ _ (derF n)) _ (
    pos_div_two _ _ H4)).
intros y H5 H6.
set
 (H7 :=
...
2: apply eq_symmetric_unfolded;
    apply
     (abs_Taylor_Rem_char realline H F a Ha f derF n
        (Derivative_n_imp_Diffble_n _ _ _ _ _ (derF n)) x
        (contin_imp_inc _ _ _ _ (included_imp_Continuous _ _ H3 _ _ _ Hinc) _
           Hx)
...
set
 ...
 in *.
eapply leEq_wdl.
apply HN with (n := n) (w := y) (z := x) (Hw := CI) (Hz := H9);
 auto with arith.
...
apply
 seq_conv_imp_fun_conv
  with (x := fun n : nat => r[^]n[/] _[//]nring_fac_ap_zero _ n).
clear Hinc Hab b a.
...
unfold Fscalmult in |- *.
apply
 conv_fun_seq'_wdr'_IR
  with (contF := Continuous_mult _ _ _ (Continuous_const _ Zero) H0).
FEQ.
apply
 fun_Lim_seq_mult'_IR
  with
    ...
    (g := fun n : nat => F)
    (contg := fun n : nat => H0)
    (contF := Continuous_const realline Zero)
    (contG := H0).
