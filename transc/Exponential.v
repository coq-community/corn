...
(**
$e^0=1$#e<sup>0</sup>=1# and $e^1=e$#e<sup>1</sup>=e#, where [e] was defined a long time ago.
*)
...
AStepr ((One[/] _[//]nring_fac_ap_zero IR n)[*]One); apply mult_wd_rht.
AStepl ((One[+][--]ZeroR)[^]n).
eapply eq_transitive_unfolded.
...
(**
Negative numbers are projected into the interval $[0,1]$#[0,1]#.
*)
...
Lemma Exp_unique_lemma :
 forall H (F : PartIR),
 Derivative realline H F F -> forall n : nat, Derivative_n n realline H F F.
...
Lemma Exp_bnd : Taylor_bnd (fun n : nat => Expon).
...
Lemma Exp_unique :
 forall F : PartIR,
 Derivative realline CI F F ->
 (forall H1, F Zero H1[=]One) -> Feq realline Expon F.
intros F H H0.
...
Lemma Exp_plus_pos :
 forall z : IR, Zero[<]z -> forall x : IR, Exp (x[+]z)[=]Exp x[*]Exp z.
intros z H x.
set
 (F :=
...
eapply eq_transitive_unfolded.
 apply Exp_plus_pos.
 2: AStepr (Exp x[*](Exp y[*]Exp z)); apply mult_wd_rht; apply Exp_plus_pos;
     auto.
unfold z in |- *.
...
elim (cotrans_less _ _ _ H4 x); intro H5.
 elim (cotrans_less _ _ _ H3 x); intro H6.
  apply Exp_pos'; auto.
...
Lemma Exp_minus :
 forall x y : IR, Exp (x[-]y)[=](Exp x[/] _[//]Exp_ap_zero y).
intros x y.
...
Lemma Exp_inv' :
 forall x y : IR, y[=][--]x -> Exp y[=](One[/] _[//]Exp_ap_zero x).
intros x y Hxy.
Step_final (Exp [--]x).
Qed.

Lemma Exp_minus' :
 forall x y z : IR, z[=]x[-]y -> Exp z[=](Exp x[/] Exp y[//]Exp_ap_zero _).
intros x y z H.
Step_final (Exp (x[-]y)).
Qed.
...
The logarithm is a continuous function with derivative $\frac1x$#1/x#.
*)
...
(**
The logarithm is strongly extensional and well defined.
*)
...
Lemma Log_mult :
 forall (x y : IR) Hx Hy Hxy, Log (x[*]y) Hxy[=]Log x Hx[+]Log y Hy.
...
Lemma Log_mult' :
 forall (x y z : IR) Hx Hy Hz, z[=]x[*]y -> Log z Hz[=]Log x Hx[+]Log y Hy.
intros.
...
(**
$\log(e^x)=x$#log(e<sup>x</sup>)=x# for all [x], both as a numerical and as a functional equation.
*)
...
Lemma Log_Exp : forall (x : IR) H, Log (Exp x) H[=]x.
intros x H.
...
Lemma Exp_Log_lemma :
 forall (x y : IR) Hx Hy, Zero[=]Log y Hy[-]Log x Hx -> y[<=]x.
...
Lemma Exp_Log : forall (x : IR) H, Exp (Log x H)[=]x.
intros x H.
...
Lemma Log_recip_char :
 forall (x : IR) Hx Hx' Hx'', Log (One[/] x[//]Hx) Hx'[+]Log x Hx''[=]Zero.
intros x Hx Hx' Hx''.
...
Lemma Log_recip :
 forall (x : IR) Hx Hx' Hx'', Log (One[/] x[//]Hx) Hx'[=][--](Log x Hx'').
intros x Hx Hx' Hx''.
...
Lemma Log_recip' :
 forall (x y : IR) Hx Hx' Hy,
 y[=](One[/] x[//]Hx) -> Log y Hy[=][--](Log x Hx').
intros x y Hx Hx' Hy H.
Step_final (Log (One[/] _[//]Hx) (recip_resp_pos _ _ Hx Hx')).
Qed.

Lemma Log_div :
 forall (x y : IR) Hx Hy Hy' Hxy,
 Log (x[/] y[//]Hy') Hxy[=]Log x Hx[-]Log y Hy.
