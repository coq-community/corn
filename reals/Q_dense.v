...
intro a.
exact
 (cotrans_analyze x (q1[+](q2[-]q1) [/]ThreeNZ) (q2[-](q2[-]q1) [/]ThreeNZ)
    (shrink23 q1 q2 a)).
intro.
exact Zero.
intro.
...
intro.
exact
 (Build_Interval _ (fstT i[+](sndT i[-]fstT i) [/]ThreeNZ, sndT i)
    (shrink24 (fstT i) (sndT i) pi)).
intro.
exact
 (Build_Interval _ (fstT i, sndT i[-](sndT i[-]fstT i) [/]ThreeNZ)
...
Proof.
 ...
 exact H.
 cut
  (if_cotrans x (Build_Interval Q_as_COrdField i pi) =
   Build_Interval Q_as_COrdField
     (fstT i[+](sndT i[-]fstT i) [/]ThreeNZ, sndT i)
     (shrink24 (fstT i) (sndT i) pi)).
 intro H1.
 ...
 cut
  (if_cotrans x (Build_Interval Q_as_COrdField i pi) =
   Build_Interval Q_as_COrdField
     (fstT i, sndT i[-](sndT i[-]fstT i) [/]ThreeNZ)
     (shrink13 (fstT i) (sndT i) pi)).
 intro H1.
...
(*
  ...
  apply
   mult_cancel_less
    with ((Three:Q_as_COrdField)[*]nring (S m)[*]nring (S (S m))). 
  apply mult_resp_pos.
  apply mult_resp_pos.
  apply pos_three.
  apply pos_nring_S.
...
Proof.
 ...
 cut
  {n : nat |
  (start_r x[-]start_l x[/] e[//]Greater_imp_ap _ e Zero H)[<]nring n}.
 intro H0.
 case H0.
 intro N.
 intro.
 ...
 apply
  AbsSmall_leEq_trans
   with
     (start_r x[-]start_l x[/] nring (S (S (N + 3)))[//]
...
Proof.
 intro.
 change
   (Cauchy_prop (fun m : nat => inj_Q R1 (CS_seq _ (G_as_CauchySeq x) m)))
  in |- *.
 apply inj_Q_Cauchy.
Qed.
...
Proof.
 ...
 cut
  {n : nat |
  (inj_Q R1 (start_r x[-]start_l x)[/] e[//]Greater_imp_ap _ e Zero H)[<]
  nring n}.
 intro H0.
 case H0.
 intro N.
 ...
 apply
  less_transitive_unfolded
   with
     (y := inj_Q R1
             ((start_r x[-]start_l x)[/]nring (R:=Q_as_COrdField) (S N)[//]
              nringS_ap_zero _ N)).
 apply inj_Q_less.
 apply
  mult_cancel_less
   with (nring (R:=Q_as_COrdField) (S (S (N + 3)))[*]nring (S N)).
 apply mult_resp_pos.
 apply pos_nring_S.
 apply pos_nring_S.
 ...
 apply
  AbsSmall_leEq_trans
   with
     (e1 := inj_Q R1
              ((start_r x[-]start_l x)[/]nring (S m)[//]
               nringS_ap_zero Q_as_COrdField m)).
 apply inj_Q_leEq.

 apply
  mult_cancel_leEq
   with (nring (R:=Q_as_COrdField) (S (S (N + 3)))[*]nring (S m)). 
 apply mult_resp_pos.
 apply pos_nring_S.
 apply pos_nring_S.
