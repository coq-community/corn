...
intros.
unfold Max_seq in |- *.
elim cotrans_less_unfolded; intro H; simpl in |- *.
 left; split; Algebra.
right; split; Algebra.
Qed.
...
exists k; intros m H.
unfold Max_seq in |- *.
elim cotrans_less_unfolded; intro Hm; simpl in |- *;
 elim cotrans_less_unfolded; intro Hk; simpl in |- *.
...
- $x\leq \maxx (x,y)$#x&le; max (x,y)#,
- $x\leq \maxx (y,x)$#x&le; max (y,x)#,
- $z<\maxx(x,y) \Rightarrow z<x \vee z<y$#z&lt; max (x,y) => z&lt;x \/ z&lt;y#.
...
Lemma MAX_strong_ext : bin_op_strong_ext _ MAX.
...
Definition Max := Build_CSetoid_bin_op _ MAX MAX_strong_ext.

Lemma max_wd_unfolded :
 forall x y x' y' : IR, x[=]x' -> y[=]y' -> Max x y[=]Max x' y'.
cut (bin_op_well_def _ MAX); [ intro | apply MAX_well_def ].
red in H.
red in H.
...
elim (cotrans_less_unfolded _ _ _ H4 (Max x y)); intros H5.
...
Lemma equiv_imp_eq_max :
 forall x x' m : IR,
 (forall y : IR, x[<]y -> x'[<]y -> m[<]y) ->
 (forall y : IR, m[<]y -> x[<]y) ->
 (forall y : IR, m[<]y -> x'[<]y) -> Max x x'[=]m.
intros.
...
Lemma Max_minus_eps_leEq :
 forall x y e : IR, Zero[<]e -> {Max x y[-]e[<=]x} + {Max x y[-]e[<=]y}.
intros.
...
Lemma min_wd_unfolded :
 forall x y a b : IR,
 x[=]a /\ y[=]b -> [--](Max [--]x [--]y)[=][--](Max [--]a [--]b).
intros.
apply un_op_wd_unfolded.
apply bin_op_wd_unfolded; apply un_op_wd_unfolded; elim H; trivial.
Qed.

Lemma min_wd : bin_op_well_def _ MIN.
red in |- *.
red in |- *.
intro.
unfold MIN in |- *.
...
Lemma min_strext_unfolded :
 forall x y a b : IR,
 [--](Max [--]x [--]y)[#][--](Max [--]a [--]b) -> x[#]a or y[#]b.
intros.
cut ([--]x[#][--]a or [--]y[#][--]b).
intros H0.
elim H0.
intro.
left; apply un_op_strext_unfolded with (cg_inv (c:=IR)); assumption.
intro.
right; apply un_op_strext_unfolded with (cg_inv (c:=IR)); assumption.
apply bin_op_strext_unfolded with Max.
apply un_op_strext_unfolded with (cg_inv (c:=IR)); assumption.
Qed.

Lemma min_strext : bin_op_strong_ext _ MIN.
red in |- *.
red in |- *.
unfold MIN in |- *.
intros.
...
Definition Min : CSetoid_bin_op IR :=
  Build_CSetoid_bin_op IR (fun x y : IR => [--](Max [--]x [--]y)) min_strext.

Lemma Min_leEq_lft : forall x y : IR, Min x y[<=]x.
intros.
unfold Min in |- *; simpl in |- *.
RStepr ([--][--]x).
apply inv_resp_leEq.
apply lft_leEq_Max.
Qed.
...
Lemma equiv_imp_eq_min :
 forall x x' m : IR,
 (forall y : IR, y[<]x -> y[<]x' -> y[<]m) ->
 (forall y : IR, y[<]m -> y[<]x) ->
 (forall y : IR, y[<]m -> y[<]x') -> Min x x'[=]m.
intros.
unfold Min in |- *; simpl in |- *.
AStepr ([--][--]m).
apply un_op_wd_unfolded.
...
Lemma leEq_Min_plus_eps :
 forall x y e : IR, Zero[<]e -> {x[<=]Min x y[+]e} + {y[<=]Min x y[+]e}.
intros.
...
Lemma ABSIR_strong_ext : un_op_strong_ext _ ABSIR.
...
Lemma ABSIR_well_def : un_op_well_def _ ABSIR.
...
Definition AbsIR : CSetoid_un_op IR :=
  Build_CSetoid_un_op _ ABSIR ABSIR_strong_ext.

Lemma AbsIR_wd : forall x y : IR, x[=]y -> AbsIR x[=]AbsIR y.
Algebra.
Qed.

Lemma Abs_wd_lft_unfolded :
 forall x y e : IR, x[=]y -> AbsIR x[<]e -> AbsIR y[<]e.
Proof.
intros.
...
Lemma Abs_wd_rht_unfolded :
 forall x y e : IR, x[=]y -> e[<]AbsIR x -> e[<]AbsIR y.
Proof.
intros.
...
apply ap_well_def_rht_unfolded with ZeroR.
assumption.
...
Lemma AbsIR_eq_AbsSmall :
 forall x e : IR, [--]e[<=]x -> x[<=]e -> AbsSmall e x.
intros.
unfold AbsSmall in |- *.
auto.
Qed.
...
Lemma AbsSmall_transitive :
 forall e x y : IR, AbsSmall e x -> AbsIR y[<=]AbsIR x -> AbsSmall e y.
intros.
...
Lemma less_AbsIR :
 forall x y : IR, Zero[<]x -> x[<]AbsIR y -> x[<]y or y[<][--]x.
intros x y H H0.
simpl in H0.
unfold ABSIR in H0.
cut (x[<]y or x[<][--]y).
...
Lemma leEq_distr_AbsIR :
 forall x y : IR, Zero[<]x -> x[<=]AbsIR y -> {x[<=]y} + {y[<=][--]x}.
intros.
...
2: RStepl ((x [/]TwoNZ) [/]TwoNZ); apply pos_div_two'; apply pos_div_two;
    auto.
rename X3 into H4.
elim (cotrans_less_unfolded _ _ _ H4 e); intro.
...
Lemma AbsIR_approach_zero :
 forall x : IR, (forall e : IR, Zero[<]e -> AbsIR x[<=]e) -> x[=]Zero.
intros.
apply leEq_imp_eq.
red in |- *; apply approach_zero_weak.
intros e H0.
...
Lemma part_function_Max_strext :
 forall (x y : IR) (Hx : Conj P Q x) (Hy : Conj P Q y),
 Max (F x (prj1 _ _ _ _ Hx)) (G x (prj2 _ _ _ _ Hx))[#]
 Max (F y (prj1 _ _ _ _ Hy)) (G y (prj2 _ _ _ _ Hy)) -> 
 x[#]y.
intros. rename X into H.
elim (bin_op_strext _ _ _ _ _ _ H).
exact (pfstrx _ F _ _ _ _).
exact (pfstrx _ G _ _ _ _).
Qed.

Definition FMax :=
  Build_PartFunct IR _ (conj_wd _ _ _ (dom_wd _ _) (dom_wd _ _))
    (fun (x : IR) (Hx : Conj P Q x) =>
     Max (F x (prj1 _ _ _ _ Hx)) (G x (prj2 _ _ _ _ Hx)))
    part_function_Max_strext.
...
Lemma FMin_char :
 forall (x : IR) Hx Hx' Hx'', FMin x Hx[=]Min (F x Hx') (G x Hx'').
intros.
Opaque Max.
simpl in |- *; Algebra.
Qed.
...
Lemma FAbs_char : forall (x : IR) Hx Hx', FAbs x Hx[=]AbsIR (F x Hx').
intros.
simpl in |- *; unfold ABSIR in |- *; apply MAX_well_def; Algebra.
Qed.
