...
Definition inject_Q (x : F) : R_COrdField' :=
  Build_CauchySeq _ _ (CS_seq_const _ x).
...
Lemma ing_nring' :
 forall m n : nat,
 CS_seq _ (nring (R:=R_COrdField') n) m[=]
 CS_seq _ (inject_Q (nring (R:=F) n)) m.
...
Lemma ing_nring :
 forall n : nat, nring (R:=R_COrdField') n[=]inject_Q (nring (R:=F) n).
...
Lemma ing_three :
 forall x : F, inject_Q x [/]ThreeNZ[=]inject_Q (x [/]ThreeNZ).
...
Lemma ing_n :
 forall (x : F) (n : nat) (H1 H2 : nring n[#]Zero),
 (inject_Q x[/] nring (R:=R_COrdField') n[//]H2)[=]
 inject_Q (x[/] nring (R:=F) n[//]H1).
...
Theorem expand_Q_R :
 forall (x : R_COrdField') (e : F),
 Zero[<]e ->
 forall N : nat,
 (forall m : nat,
  N <= m -> AbsSmall (e [/]FourNZ) (CS_seq F x m[-]CS_seq F x N)) ->
 forall m : nat,
 N <= m -> AbsSmall (inject_Q e) (inject_Q (CS_seq F x m)[-]x).
Proof.
...
Lemma modulus_property :
 forall (x : R_COrdField') (M m0 m1 : nat),
 T x M <= m0 ->
 T x M <= m1 ->
...
Lemma modulus_property_2 :
 forall (x : R_COrdField') (M m : nat),
 T x M <= m -> AbsSmall (one_div_succ M) (CS_seq F x m[-]CS_seq F x (T x M)).
...
Lemma expand_Q_R_2 :
 forall (x : R_COrdField') (e : F) (N : nat),
 Zero[<]e ->
 (forall m : nat,
  N <= m -> AbsSmall (e [/]FourNZ) (CS_seq F x m[-]CS_seq F x N)) ->
...
Lemma CS_seq_diagonal :
 forall a : CauchySeq R_COrdField',
 Cauchy_prop
   (fun m : nat =>
    CS_seq F (CS_seq R_COrdField' a m) (T (CS_seq R_COrdField' a m) m)).
...
Lemma Q_dense_in_R :
 forall x : R_COrdField', Zero[<]x -> {q : F | Zero[<]q | inject_Q q[<]x}.
...
(*
 Cut (Cauchy_prop (LimR_CauchySeq (Build_CauchySeq R_COrdField' a_ pa))).
 Intro H10.
*)
...
Definition R_is_CReals :=
  Build_is_CReals R_COrdField' LimR_CauchySeq R_is_complete R_is_archemaedian.

Definition R_as_CReals :=
  Build_CReals R_COrdField' LimR_CauchySeq R_is_CReals.
