...
cut (forall z, In z (iterateN Zsucc 0%Z n) -> (0 <= z < n)%Z).
 ...
 split.
  ...
  rapply Qle_shift_div_l; auto with *.
  replace LHS with 0 by (simpl; ring).
  change (0 <= (2*a + 1)*1)%Z.
  auto with *.
 rewrite Qle_minus_iff.
 unfold f.
 replace RHS with (((r-l)*((2*S n#1) + - (2 * a + 1 # 1))) / (2 * S n # 1))
  by (field; discriminate).
...
setoid_replace ((x - (l + (r - l) * (2 * rasterize1 (S n) x + 1 # 1) / (2 * S n))) *
   (S n / (r - l)))
 with ((S n)*(x-l)/(r-l) - (2*rasterize1 (S n) x + 1 # 1)/2)
 by (field; auto).
rewrite Qmake_Qdiv.
rewrite injz_plus.
...
destruct (Qlt_le_dec_fast x r).
 exists (f (S n) (rasterize1 (S n) x)).
  abstract (
  destruct Hx as [Hlx Hxr];
  split;
  [rapply in_map;
   rewrite UniformPartitionZ;
   split;
   [apply rasterize1_boundL; auto
   |apply rasterize1_boundR; auto]
  |apply rasterize1_close;
...
set (e2':=(max 1 (Z_to_nat (z:=Qceiling ((r - l) / (2%positive * e2)))
           (CompactIntervalQ_nat e2)))).
assert (L:=UniformPartition_fine (pred e2') H).
rewrite S_predn in L;
[|intros Z; symmetry in Z;revert Z;
  apply lt_O_neq;
  unfold e2';
...
split.
 ...
 simpl in L.
 set (a:=(max 1
            (Z_to_nat (z:=Qceiling ((r - l) / (2%positive * e2)))
