...
intro x.
elim (is_Archimedes (x[+]One)); intros n Hn.
exists n.
apply less_leEq_trans with (x[+]One).
apply less_plusOne.
auto.
Qed.
...
Lemma inequality2 : forall x : OF, (Zero:OF)[<]x[^]2[-]x[+]Two. 
Proof.
 intros.
...
Lemma inequality4 : forall x : OF, [--](x[^]2)[-]x[-]Two[<](Zero:OF).
Proof.
 intros.
 apply inv_cancel_less.
 astepl (Zero:OF).
...
Lemma bound_tk1 :
 ...
 (forall m : nat, m <= n -> (Zero:OF)[<]g m) -> (Zero:OF)[<]Hum g n.
Proof.
 intros n g H.
 induction  n as [| n Hrecn].
 simpl in |- *.
...
Lemma bound_tk2 :
 forall (n : nat) (g : nat -> OF),
 (forall m : nat, m <= n -> g m[<](Zero:OF)) -> Hum g n[<]Zero.
Proof.
 intros n g H.
 induction  n as [| n Hrecn].
 simpl in |- *.
...
Proof.
 ...
 intros. 
 cut
  ((forall k : nat, k <= card -> seq k[<]y)
   or {j : nat | P (seq j) | x[<]seq j}).
 intro H0.
 case H0.
   intro c.
   left.
   ...
   case
    (bounded_quantifier_informative card)
...
Proof.
 ...
 apply eq_reflexive_unfolded.
 (* it is bounded above *)
 cut {N : nat | forall m : nat, N <= m -> AbsSmall One (g_ m[-]g_ N)}.
 intro H.
 case H.
 intro N.
 intro.
 exists (saghf g_ N[+]One).
 red in |- *.
 intros x H0 y H1.
 red in H0.
 case H0.
 ...
 case (le_ge_dec N n).
 intro H2.
 apply leEq_less_trans with (y := g_ N[+]One).
 ...
 cut
  {N : nat |
  forall m : nat, N <= m -> AbsSmall ((b[-]a) [/]TwoNZ) (g_ m[-]g_ N)}.
 intro H0.
 case H0.
 intro N.
 intros.
...
Proof.  
 ...
 case (le_ge_dec N n).
 intro.
 apply less_leEq_trans with (y := g_ N[-]One).
 apply less_transitive_unfolded with (y := kaf g_ N[-]One).
 assumption.
...
Proof.
 ...
 intros.
 cut
  {N : nat | forall m : nat, N <= m -> AbsSmall (e [/]TwoNZ) (g_ m[-]g_ N)}.
 intros H0.
 case H0.
 intro N.
 intros.
...
Proof.
 red in |- *.
 intros e H.
 case (is_Archimedes' ((Two[/] e[//]Greater_imp_ap _ e Zero H)[-]One)).
 intro k.
 intros.
 case (pg (e [/]FourNZ)).
 apply div_resp_pos.
