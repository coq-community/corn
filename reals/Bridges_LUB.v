...
Proof.
 ...
 elim
  (located (r1[+](r2[-]r1) [/]ThreeNZ) (r2[-](r2[-]r1) [/]ThreeNZ)
     (shrink23d _ _ H)).
 intro.     
 right.
 split.
...
Proof.
 ...
 exact H.
 cut
  (dif_cotrans (Build_Interval _ i pi) =
   Build_Interval _ (fstT i[+](sndT i[-]fstT i) [/]ThreeNZ, sndT i)
     (shrink24d (fstT i) (sndT i) pi)).
 ...
 exact H.
 cut
  (dif_cotrans (Build_Interval R1 i pi) =
...
Proof.
 ...
 case (le_lt_eq_dec 4 (S m) H).
  ...
  apply
   mult_cancel_less with (z := (Three:R1)[*]nring (S m)[*]nring (S (S m))). 
  apply mult_resp_pos.
  apply mult_resp_pos.
  apply pos_three.
  apply pos_nring_S.
...
Proof.
 ...
 cut
  {n : nat |
  (dstart_r[-]dstart_l[/] e[//]Greater_imp_ap _ e Zero H)[<]nring n}.
 intro H0.
 case H0.
 intro N.
 intro.
 ...
 apply
  AbsSmall_leEq_trans
   with
     (e1 := dstart_r[-]dstart_l[/] nring (S (S (N + 3)))[//]
...
Proof.
 ...
 apply
  less_wdl
   with
     (x := (One:R1)[+]
           nring (S N)[*](One[/] nring (S N)[//]nringS_ap_zero _ N)).
...
Proof.
 ...
 cut
  {n : nat |
  (Two[*](dstart_r[-]dstart_l)[/] e[//]Greater_imp_ap _ e Zero H)[<]nring n}.
 intro H0.
 case H0.
 intro N.
 intros.
 ...
 apply
  AbsSmall_leEq_trans
   with (e1 := (Two [/]ThreeNZ)[^]m[*](dstart_r[-]dstart_l)).
 apply less_leEq.
