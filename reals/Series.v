...
apply
 Cauchy_plus
  with
    (seq1 := Build_CauchySeq _ _ convX)
    (seq2 := Build_CauchySeq _ _ convY).
simpl in |- *.
unfold seq_part_sum in |- *.
intro.
...
apply
 Cauchy_mult
  with (seq2 := Build_CauchySeq _ _ convX) (seq1 := Cauchy_const c).
simpl in |- *.
unfold seq_part_sum in |- *.
intro.
...
apply
 Cauchy_minus
  with (seq1 := Cauchy_const Zero) (seq2 := Build_CauchySeq _ _ convX).
simpl in |- *.
unfold seq_part_sum in |- *.
intro.
apply eq_symmetric_unfolded;
...
2: omega.
pattern (S (max N M + k)) at 2 in |- *;
 replace (S (max N M + k)) with (S (max N M + k) - k + k).
2: omega.
apply Sum_big_shift.
intros; apply HN.
apply le_trans with (max N M); auto with arith.
...
apply pos_x.
clear m; intro m.
cut (n + (S m + S m) = S (n + S (m + m)));
 [ intro | simpl in |- *; rewrite <- plus_n_Sm; auto ].
rewrite H.
apply
 leEq_wdr
  with
...
apply less_leEq.
apply
 less_wdl with (One[/] _[//]Greater_imp_ap IR _ _ (pos_nring_S _ (m + m))).
