...
Definition IntegrableSetsGen (IS : IntegrationSpace) : Type
  := forall (A : (X (ElemFunc IS)) -> Prop)
    ...
    IntegrableSet A
    -> 0 < eps
    -> { B : (X (ElemFunc IS)) -> Prop
        &  prod (forall x, B x -> A x) (SetApprox A B eps) }. 

(* It is enough to truncate on generator sets to prove that
   a function is measurable. *)
(*
Lemma MeasurableGen
  : forall {IS : IntegrationSpace}
      (f : PartialFunction (X (ElemFunc IS)))
    ...
    (forall (A : (X (ElemFunc IS)) -> Prop) (Aint : IntegrableSet A)
       ...
       (eps : CRcarrier (RealT (ElemFunc IS))) (epsPos : 0 < eps),
        IntegrableFunction
          (XmaxConst (XminConst (Xmult (CharacFunc (let (B,_) := gen A eps Aint epsPos in B)) f)
                                (CR_of_Q _ (Z.pos k # 1)))
                     (CR_of_Q _ (Z.neg k # 1))))
    -> MeasurableFunction f.
Proof.
  intros IS f gen fMes. 
Admitted.
*)
