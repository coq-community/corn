...
(** *Properties of Real Number Structures
%\begin{convention}%
...
#for all k there exists N such that for all m &ge; N |seqm-seqN| &le; 1/(k+1)#
 ...
 predicate [Cauchy_Lim_prop?],
 in terms of which [Cauchy_prop?] is defined.

Variant 2 of Cauchy ([Cauchy_prop2]):

%\[\exists y. \verb!Cauchy_Lim_prop2!~seq~y\] where \[\verb!Cauchy_Lim_prop2!~seq~y := \forall \epsilon>0. \exists N. \forall m \geq N. |seq_m - y|\leq\epsilon\]%
#There exists y [Cauchy_Lim_prop2] seq y where [Cauchy_Lim_prop2] seq y := for all e>0 there exists N such that for all m &ge; N |seq_m - y| &le; e#

Note that [Cauchy_Lim_prop2] equals [seqLimit].
Variant 3 of Cauchy ([Cauchy_prop3]):

%\[\exists y. \verb!Cauchy_Lim_prop3!~seq~y\] where \[\verb!Cauchy_Lim_prop3!~seq~y := \forall k. \exists N. \forall m\geq N. |seq_m - y|\leq1/(k+1)\]%
#There exists y [Cauchy_Lim_prop3] seq y where [Cauchy_Lim_prop3] seq y := for all k  there exists N such that for all m&ge; N |seqm - y| &le; 1/(k+1)#
The following variant is more restricted.

Variant 4 of Cauchy ([Cauchy_prop4]):

%\[\exists y. \verb!Cauchy_Lim_prop4!~seq~y\] where \[\verb!Cauchy_Lim_prop4!~seq~y := \forall k. |seq_k - y|\leq1/(k+1)\]%
#There exists y [Cauchy_Lim_prop4] seq y where [Cauchy_Lim_prop4] seq y := for all k |seq_k - y| &le;1/(k+1)#
...
The inequalities are stated with $\leq$#&le;# rather than with [<] for the following reason: both formulations
are equivalent, as is well known; and, $\leq$#&le;# being a negative statement, this makes proofs sometimes easier and program extraction much more efficient.
*)

Definition Cauchy_prop1 (seq : nat -> IR) :=
  forall k : nat,
  {N : nat |
  forall m : nat, N <= m -> AbsSmall (one_div_succ k) (seq m[-]seq N)}.
...
Lemma Cauchy_complete :
 forall seq : CauchySeq IR, Cauchy_Lim_prop2 seq (Lim seq).
exact Lim_Cauchy.
Qed.

Lemma less_Lim_so_less_seq :
 forall (seq : CauchySeq IR) (y : IR),
 y[<]Lim seq -> {N : nat | forall m : nat, N <= m -> y[<]seq m}.
intros seq y H.
...
Lemma Lim_less_so_seq_less :
 forall (seq : CauchySeq IR) (y : IR),
 Lim seq[<]y -> {N : nat | forall m : nat, N <= m -> seq m[<]y}.
intros.
...
Lemma Lim_less_Lim_so_seq_less_seq :
 forall seq1 seq2 : CauchySeq IR,
 Lim seq1[<]Lim seq2 -> {N : nat | forall m : nat, N <= m -> seq1 m[<]seq2 m}.
intros.
...
Lemma less_Lim_so :
 ...
 y[<]Lim seq ->
 {eps : IR | Zero[<]eps |
 {N : nat | forall m : nat, N <= m -> y[+]eps[<]seq m}}.
intros.
...
Lemma Lim_less_so :
 ...
 Lim seq[<]y ->
 {eps : IR | Zero[<]eps |
 {N : nat | forall m : nat, N <= m -> seq m[+]eps[<]y}}.
intros.
...
Lemma leEq_seq_so_leEq_Lim :
 forall (seq : CauchySeqR) (y : IR),
 (forall i : nat, y[<=]seq i) -> y[<=]Lim seq.
intros.
unfold leEq in |- *.
intro H0.
...
Lemma str_leEq_seq_so_leEq_Lim :
 forall (seq : CauchySeq IR) (y : IR),
 (exists N : nat, (forall i : nat, N <= i -> y[<=]seq i)) -> y[<=]Lim seq.
intros.
red in |- *; intro H0.
...
Lemma Lim_leEq_Lim :
 forall seq1 seq2 : CauchySeqR,
 (forall i : nat, seq1 i[<=]seq2 i) -> Lim seq1[<=]Lim seq2.
intros.
unfold leEq in |- *.
intro H0.
...
Lemma seq_leEq_so_Lim_leEq :
 forall (seq : CauchySeqR) (y : IR),
 (forall i : nat, seq i[<=]y) -> Lim seq[<=]y.
intros.
unfold leEq in |- *.
intro H0.
...
Lemma str_seq_leEq_so_Lim_leEq :
 forall (seq : CauchySeq IR) (y : IR),
 (exists N : nat, (forall i : nat, N <= i -> seq i[<=]y)) -> Lim seq[<=]y.
intros.
red in |- *; intro H0.
...
Lemma Limits_unique :
 forall (seq : CauchySeq IR) (y : IR), Cauchy_Lim_prop2 seq y -> y[=]Lim seq.
intros seq y H.
apply not_ap_imp_eq.
...
Lemma Cauchy_prop1_prop :
 forall seq : nat -> IR, Cauchy_prop1 seq -> Cauchy_prop seq.
intros seq H.
...
Lemma Cauchy_prop2_prop :
 forall seq : nat -> IR, Cauchy_prop2 seq -> Cauchy_prop seq.
intros seq H.
unfold Cauchy_prop in |- *.
intros e H0.
...
Lemma Cauchy_prop3_prop2 :
 forall seq : nat -> IR, Cauchy_prop3 seq -> Cauchy_prop2 seq.
...
Lemma Cauchy_prop3_prop :
 forall seq : nat -> IR, Cauchy_prop3 seq -> Cauchy_prop seq.
intros.
...
Definition Build_CauchySeq1 :
  forall seq : nat -> IR, Cauchy_prop1 seq -> CauchySeqR.
intros.
...
Lemma Cauchy_prop4_prop3 :
 forall seq : nat -> IR, Cauchy_prop4 seq -> Cauchy_prop3 seq.
...
Lemma Cauchy_prop4_prop :
 forall seq : nat -> IR, Cauchy_prop4 seq -> Cauchy_prop seq.
intros.
...
Definition Build_CauchySeq4 :
  forall seq : nat -> IR, Cauchy_prop4 seq -> CauchySeqR.
intros.
unfold CauchySeqR in |- *.
apply Build_CauchySeq with seq.
...
Definition Build_CauchySeq4_y :
  forall (seq : nat -> IR) (y : IR), Cauchy_Lim_prop4 seq y -> CauchySeqR.
intros.
...
Lemma Lim_CauchySeq4 :
 forall (seq : nat -> IR) (y : IR) (H : Cauchy_Lim_prop4 seq y),
...
Definition Build_CauchySeq2 :
  forall seq : nat -> IR, Cauchy_prop2 seq -> CauchySeqR.
intros.
unfold CauchySeqR in |- *.
apply Build_CauchySeq with seq.
apply Cauchy_prop2_prop.
...
Definition Build_CauchySeq2_y :
  forall (seq : nat -> IR) (y : IR), Cauchy_Lim_prop2 seq y -> CauchySeqR.
intros.
...
Lemma Lim_CauchySeq2 :
 forall (seq : nat -> IR) (y : IR) (H : Cauchy_Lim_prop2 seq y),
...
Lemma Cauchy_prop_wd :
 forall seq1 seq2 : nat -> IR,
 Cauchy_prop seq1 -> (forall n : nat, seq1 n[=]seq2 n) -> Cauchy_prop seq2.
...
Lemma Cauchy_Lim_prop2_wd :
 forall (seq1 seq2 : nat -> IR) (c : IR),
 Cauchy_Lim_prop2 seq1 c ->
 (forall n : nat, seq1 n[=]seq2 n) -> Cauchy_Lim_prop2 seq2 c.
...
Lemma Cauchy_Lim_plus :
 forall (seq1 seq2 : nat -> IR) (y1 y2 : IR),
 Cauchy_Lim_prop2 seq1 y1 ->
 Cauchy_Lim_prop2 seq2 y2 ->
 Cauchy_Lim_prop2 (fun n : nat => seq1 n[+]seq2 n) (y1[+]y2).
...
Lemma Cauchy_plus :
 forall seq1 seq2 : CauchySeqR, Cauchy_prop (fun n : nat => seq1 n[+]seq2 n).
intros.
apply Cauchy_prop2_prop.
unfold Cauchy_prop2 in |- *.
...
Lemma Lim_plus :
 ...
 Lim
   (Build_CauchySeq IR (fun n : nat => seq1 n[+]seq2 n)
      (Cauchy_plus seq1 seq2))[=]Lim seq1[+]Lim seq2.
intros.
apply eq_symmetric_unfolded.
apply Limits_unique.
simpl in |- *.
...
Lemma Cauchy_Lim_inv :
 forall (seq : nat -> IR) (y : IR),
 Cauchy_Lim_prop2 seq y ->
 Cauchy_Lim_prop2 (fun n : nat => [--](seq n)) [--]y.
intros seq y H.
unfold Cauchy_Lim_prop2 in |- *.
intros eps H0.
...
Lemma Cauchy_inv :
 forall seq : CauchySeqR, Cauchy_prop (fun n : nat => [--](seq n)).
intros.
apply Cauchy_prop2_prop.
unfold Cauchy_prop2 in |- *.
exists ([--](Lim seq)).
...
Lemma Lim_inv :
 forall seq : CauchySeqR,
 Lim (Build_CauchySeq IR (fun n : nat => [--](seq n)) (Cauchy_inv seq))[=]
 [--](Lim seq).
intros.
apply eq_symmetric_unfolded.
apply Limits_unique.
...
Lemma Lim_minus :
 ...
 Lim
   (Build_CauchySeq IR (fun n : nat => seq1 n[-]seq2 n)
      (Cauchy_minus seq1 seq2))[=]Lim seq1[-]Lim seq2.
intros.
apply eq_symmetric_unfolded.
apply Limits_unique.
simpl in |- *.
...
Lemma Lim_mult :
 ...
 Lim
   (Build_CauchySeq IR (fun n : nat => seq1 n[*]seq2 n)
      (Cauchy_mult seq1 seq2))[=]Lim seq1[*]Lim seq2.
intros.
apply eq_symmetric_unfolded.
apply Limits_unique.
simpl in |- *.
