...
Lemma cc_ap_zero :
 forall P : CC -> Prop,
 (forall a b : IR, a[#]Zero -> P (a[+I*]b)) ->
 (forall a b : IR, b[#]Zero -> P (a[+I*]b)) -> forall c : CC, c[#]Zero -> P c.
...
Lemma imag_to_real :
 forall a b a' b' : IR, a'[+I*]b'[=](a[+I*]b)[*]II -> a[#]Zero -> b'[#]Zero.
do 5 intro. intro H0.
cut (b'[=]a); intros.
(* AStepl a. *)
...
Lemma nroot_I_nexp_aux :
 forall n : nat, odd n -> {m : nat | n * n = 4 * m + 1}.
intros n on.
...
Definition nroot_I (n : nat) (on : odd n) : CC := II[^]n.

Lemma nroot_I_nexp : forall (n : nat) (on : odd n), nroot_I _ on[^]n[=]II.
intros.
unfold nroot_I in |- *.
(* AStepl II[^](mult n n). *)
...
Lemma nroot_minus_I_nexp :
 forall (n : nat) (on : odd n), nroot_minus_I n on[^]n[=][--]II.
intros.
...
apply pos_square.
(* AStepr Zero[*]Half. *)
  apply ap_well_def_rht_unfolded with (ZeroR[*]Half).
...
(**
We define the nth root of a complex number with a non zero real part.
*)
...
(**
%\begin{convention}%
Let [n:nat] and [c,z:CC] and [c_:(c [#] Zero)].
%\end{convention}%
*)
Variable n : nat.
Variables c z : CC.
...
intros.
apply Cconditional_square_eq; try assumption.
exact TwoCC_ap_zero.
...
Lemma nrCC3_a1 :
 forall (p : cpoly CC) (r : IR), (Im_poly p) ! r[=]Im p ! (cc_IR r).
intros.
elim p; intros.
unfold Im_poly in |- *.
(* AStepl (Zero::IR). *)
...
(**
%\begin{convention}%
Let [a,b:IR], [b_:(b [#] Zero)] and [n:nat].
%\end{convention}%
*)
Variables a b : IR.
Hypothesis b_ : b[#]Zero.
...
Lemma nrCC3_a7 :
 forall r : IR, nrCC3_poly' ! (cc_IR r)[=](r[+I*]One)[^]n[*](a[+I*][--]b).
intros.
unfold nrCC3_poly' in |- *.
...
Lemma nrootCC_3_ :
 forall r : IR, nrootCC_3_poly ! r[=]Im ((r[+I*]One)[^]n[*](a[+I*][--]b)).
intros.
...
unfold nrootCC_3'_poly in |- *.
apply (degree_invus_lft _ (_C_ c) (_X_[^]n) 0 n).
apply degree_le_c_.
...
(**
%\begin{convention}%
Let [a,b:IR] and [b_:(b [#] Zero)] and [n:nat] and
[on:(odd n)].
%\end{convention}%
*)
Variables a b : IR.
...
Lemma nrCC4_a1 :
 {r : IR | (r[+I*]One)[^]n[*]CC_conj c[-](r[+I*][--]One)[^]n[*]c[=]Zero}.
...
Variable r : IR.
Hypothesis
  r_property : (r[+I*]One)[^]n[*]CC_conj c[-](r[+I*][--]One)[^]n[*]c[=]Zero.
Variable y2 : IR.
...
Lemma nrCC4_a10 :
 forall c : CC, {z : CC | z[^]n[=]c or z[^]n[=][--]c} -> {z : CC | z[^]n[=]c}.
intros c0 H.
elim H. intros x H0.
elim H0; intro H1.
exists x. assumption.
...
(**
%\begin{convention}%
Let [a,b:IR] and [n:nat]
with [a [#] Zero] and [(odd n)].
%\end{convention}%
*)
Variables a b : IR.
Hypothesis a_ : a[#]Zero.
...
Lemma nrootCC_4 :
 forall c : CC, c[#]Zero -> forall n : nat, odd n -> {z : CC | z[^]n[=]c}.
intros.
pattern c in |- *.
apply C_cc_ap_zero; try assumption; intros.
...
Lemma C_nrCC_5a1 :
 ...
 (forall n : nat, 0 < n -> P n -> P (double n)) ->
 forall n : nat, 0 < n -> P n.
...
Lemma nrCC_5a4 :
 forall n : nat, 0 < n -> {z : CC | z[^]n[=]c} -> {z : CC | z[^]double n[=]c}.
intros n H H0.
elim H0. intros x H1.
...
(* Step_final x[^]n. *)
  ...
  exact H1.
apply (un_op_strext _ (nexp_op (R:=CC) n)).
(* AStepl c. *)
  apply ap_well_def_lft_unfolded with c.
...
Definition CnrootCC :
  forall c : CC, c[#]Zero -> forall n : nat, 0 < n -> {z : CC | z[^]n[=]c} :=
  nrootCC_5.