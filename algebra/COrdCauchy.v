...
Definition Cauchy_prop (g : nat -> R) : CProp :=
  forall e : R,
  Zero[<]e -> {N : nat | forall m : nat, N <= m -> AbsSmall e (g m[-]g N)}.
...
Record CauchySeq : Type := 
  {CS_seq :> nat -> R; CS_proof : Cauchy_prop CS_seq}.

Definition SeqLimit (seq : nat -> R) (lim : R) : CProp :=
  forall e : R,
  Zero[<]e -> {N : nat | forall m : nat, N <= m -> AbsSmall e (seq m[-]lim)}.
...
Theorem CS_seq_bounded :
 forall g : nat -> R,
 Cauchy_prop g ->
 {K : R | Zero[<]K | {N : nat | forall m : nat, N <= m -> AbsSmall K (g m)}}.
 intros g Hg.
...
Lemma CS_seq_const : forall c : R, Cauchy_prop (fun n : nat => c).
exists 0.
...
(**
%\begin{convention}%
Assume [f] and [g] are Cauchy sequences on [R].
%\end{convention}%
*)
...
Lemma CS_seq_plus : Cauchy_prop (fun m : nat => f m[+]g m).
 unfold Cauchy_prop in |- *.
 intros.
...
Lemma CS_seq_inv : Cauchy_prop (fun n : nat => [--](f n)).
red in |- *; intros e H.
...
Lemma CS_seq_mult : Cauchy_prop (fun n : nat => f n[*]g n).
red in |- *; intros e He.
...
Lemma maj_upto_eps :
 forall (F : COrdField) (a : nat -> F) (n : nat) (eps : F),
 0 < n ->
 Zero[<]eps ->
 {k : nat |
 1 <= k /\ k <= n /\ (forall i : nat, 1 <= i -> i <= n -> a i[-]eps[<=]a k)}.
...
intro H.
elim (cotrans_less_unfolded _ _ _ H (a k)); intro H4.
 exists k.
...
Lemma mult_AbsSmall :
 forall x y X Y : R, AbsSmall X x -> AbsSmall Y y -> AbsSmall (X[*]Y) (x[*]y).
unfold AbsSmall in |- *.
intros.
...
intros x y H H0.
elim (cotrans_less_unfolded _ _ _ (half_3 _ _ H) y); intro.
exists (Half[*]x).
...
unfold AbsSmall in |- *.
cut (x[<]x[+]One). intro H.
elim (cotrans_less_unfolded _ x (x[+]One) H [--]x); intro.
exists ([--]x[+]One).
...
Lemma resp_less_char' :
 forall (P : R -> CProp) (f : forall x : R, P x -> R) x y Hx Hy,
 (x[#]y -> f x Hx[#]f y Hy) ->
...
is monotonous iff $f(i)<f(i+1)$ #f(i)<f(i+1)# for every $i$ #i#.

Also, strictly monotonous functions are injective.
*)

Lemma local_mon_imp_mon :
 forall f : nat -> R,
 (forall i : nat, f i[<]f (S i)) -> forall i j : nat, i < j -> f i[<]f j.
simple induction j.
...
Lemma local_mon_imp_mon_lt :
 forall (n : nat) (f : forall i : nat, i < n -> R),
 (forall (i : nat) H H', f i H[<]f (S i) H') ->
...
Lemma local_mon_imp_mon'_lt :
 forall (n : nat) (f : forall i : nat, i < n -> R),
 (forall (i : nat) H H', f i H[<]f (S i) H') ->
 nat_less_n_fun _ _ f -> forall (i j : nat) Hi Hj, i <= j -> f i Hi[<=]f j Hj.
intros.
...
Lemma mon_imp_inj_lt :
 forall (n : nat) (f : forall i : nat, i < n -> R),
 (forall (i j : nat) Hi Hj, i < j -> f i Hi[<]f j Hj) ->
 forall (i j : nat) Hi Hj, f i Hi[=]f j Hj -> i = j.
intros.
cut (~ i <> j); intro.
...
Lemma part_mon_imp_mon' :
 forall (F : PartFunct R) (I : R -> CProp),
 (forall x : R, I x -> Dom F x) ->
 (forall x y : R, I x -> I y -> x[<]y -> forall Hx Hy, F x Hx[<]F y Hy) ->
 forall x y : R, I x -> I y -> x[<=]y -> forall Hx Hy, F x Hx[<=]F y Hy.
intros.
intro.
cut (x[=]y); intros.
