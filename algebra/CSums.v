...
Fixpoint Sumx (n : nat) : (forall i : nat, i < n -> G) -> G :=
  match n return ((forall i : nat, i < n -> G) -> G) with
  | O => fun _ : forall i : nat, i < 0 -> G => Zero:G
  | S m =>
      fun f : forall i : nat, i < S m -> G =>
      Sumx m (fun (i : nat) (l : i < m) => f i (lt_S _ _ l))[+]
      f m (lt_n_Sn m)
  end.
...
$0$ #0# when the
input is greater than or equal to $i$ #i#.
*)

Definition part_tot_nat_fun (n : nat) (f : forall i : nat, i < n -> G) :
  nat -> G.
intros n f i.
elim (le_lt_dec n i).
...
Lemma part_tot_nat_fun_ch1 :
 forall (n : nat) (f : forall i : nat, i < n -> G),
 nat_less_n_fun _ _ f ->
 forall (i : nat) (Hi : i < n), part_tot_nat_fun n f i[=]f i Hi.
intros n f Hf i Hi.
...
Lemma part_tot_nat_fun_ch2 :
 forall (n : nat) (f : forall i : nat, i < n -> G) (i : nat) (Hi : n <= i),
 part_tot_nat_fun n f i[=]Zero.
intros n f i Hi.
unfold part_tot_nat_fun in |- *.
elim le_lt_dec; intro.
 simpl in |- *; Algebra.
...
Definition Sum1 (m n : nat) (f : nat -> G) : G := Sum0 n f[-]Sum0 m f.

Definition Sum (m n : nat) : (nat -> G) -> G := Sum1 m (S n).
(* Sum i=m..n *)

(** [Sum2] is similar to [Sum1], but does not require the summand to be
defined outside where it is being added. *)
Definition Sum2 (m n : nat) (h : forall i : nat, m <= i -> i <= n -> G) : G.
intros m n h.
apply (Sum m n).
intro i.
...
Lemma Sum_one : forall (n : nat) (f : nat -> G), Sum n n f[=]f n.
intros n f.
unfold Sum in |- *.
unfold Sum1 in |- *.
simpl in |- *.
...
Lemma Sum_empty :
 forall (n : nat) (f : nat -> G), 0 < n -> Sum n (pred n) f[=]Zero.
intros n f H.
unfold Sum in |- *.
...
Lemma Sum_Sum :
 forall (l m n : nat) (f : nat -> G), Sum l m f[+]Sum (S m) n f[=]Sum l n f.
intros l m n f.
unfold Sum in |- *.
unfold Sum1 in |- *.
...
Lemma Sum_first :
 forall (m n : nat) (f : nat -> G), Sum m n f[=]f m[+]Sum (S m) n f.
intros m n f.
unfold Sum in |- *.
unfold Sum1 in |- *.
...
Lemma Sum_last :
 forall (m n : nat) (f : nat -> G), Sum m (S n) f[=]Sum m n f[+]f (S n).
intros m n f.
unfold Sum in |- *.
unfold Sum1 in |- *.
simpl in |- *.
...
Lemma Sum_last' :
 forall (m n : nat) (f : nat -> G),
 0 < n -> Sum m n f[=]Sum m (pred n) f[+]f n.
...
Lemma Sum0_strext :
 forall f g (n : nat), Sum0 n f[#]Sum0 n g -> {i : nat | i < n | f i[#]g i}.
intros f g n H.
...
Lemma Sumx_strext :
 forall (n : nat) (f g : forall i : nat, i < n -> G),
 nat_less_n_fun _ _ f ->
...
elim (bin_op_strext_unfolded _ _ _ _ _ _ H1); clear H1; intro H1.
 cut (nat_less_n_fun _ _ (fun (i : nat) (l : i < n) => f i (lt_S _ _ l)));
  [ intro H2 | red in |- *; intros; apply H; assumption ].
 cut (nat_less_n_fun _ _ (fun (i : nat) (l : i < n) => g i (lt_S _ _ l)));
...
Lemma Sum0_strext' :
 forall f g (n : nat), Sum0 n f[#]Sum0 n g -> {i : nat | f i[#]g i}.
intros f g n H.
elim (Sum0_strext _ _ _ H); intros i Hi Hi'; exists i; auto.
Qed.

Lemma Sum_strext' :
 forall f g (m n : nat), Sum m n f[#]Sum m n g -> {i : nat | f i[#]g i}.
intros f g m n H.
...
Lemma Sum0_wd :
 forall (m : nat) (f f' : nat -> G),
 (forall i : nat, f i[=]f' i) -> Sum0 m f[=]Sum0 m f'.
intros m f f' H.
elim m; simpl in |- *; Algebra.
Qed.

Lemma Sum_wd :
 forall (m n : nat) (f f' : nat -> G),
 (forall i : nat, f i[=]f' i) -> Sum m n f[=]Sum m n f'.
intros m n f f' H.
unfold Sum in |- *.
unfold Sum1 in |- *.
unfold cg_minus in |- *.
...
Lemma Sumx_wd :
 forall (n : nat) (f g : forall i : nat, i < n -> G),
 (forall (i : nat) (H : i < n), f i H[=]g i H) -> Sumx _ f[=]Sumx _ g.
...
Lemma Sum_wd' :
 ...
 m <= S n ->
 forall f f' : nat -> G,
 (forall i : nat, m <= i -> i <= n -> f i[=]f' i) -> Sum m n f[=]Sum m n f'.
...
Lemma Sum2_wd :
 forall m n : nat,
 m <= S n ->
 forall f g : forall i : nat, m <= i -> i <= n -> G,
 (forall (i : nat) Hm Hn, f i Hm Hn[=]g i Hm Hn) -> Sum2 _ _ f[=]Sum2 _ _ g.
intros m n H f g H0.
unfold Sum2 in |- *.
apply Sum_wd'.
 assumption.
intros i H1 H2.
elim le_lt_dec; intro H3;
...
Lemma Sum0_plus_Sum0 :
 forall (f g : nat -> G) (m : nat),
 Sum0 m (fun i : nat => f i[+]g i)[=]Sum0 m f[+]Sum0 m g.
intros f g m.
elim m.
 simpl in |- *; Algebra.
intros n H.
...
Lemma Sum_plus_Sum :
 forall (f g : nat -> G) (m n : nat),
 Sum m n (fun i : nat => f i[+]g i)[=]Sum m n f[+]Sum m n g.
intros f g m n.
unfold Sum in |- *.
unfold Sum1 in |- *.
...
Lemma Sumx_plus_Sumx :
 forall (n : nat) (f g : forall i : nat, i < n -> G),
 Sumx _ f[+]Sumx _ g[=]Sumx _ (fun (i : nat) (Hi : i < n) => f i Hi[+]g i Hi).
intro n; induction  n as [| n Hrecn].
...
apply
 eq_transitive_unfolded
  with
    (Sumx _ (fun (i : nat) (l : i < n) => f i (lt_S i n l))[+]
     Sumx _ (fun (i : nat) (l : i < n) => g i (lt_S i n l))[+]
...
Lemma Sum2_plus_Sum2 :
 forall m n : nat,
 m <= S n ->
 forall f g : forall i : nat, m <= i -> i <= n -> G,
 Sum2 _ _ f[+]Sum2 _ _ g[=]Sum2 _ _ (fun i Hm Hn => f i Hm Hn[+]g i Hm Hn).
intros m n H f g.
...
Lemma inv_Sum :
 forall (f : nat -> G) (m n : nat),
 Sum m n (fun i : nat => [--](f i))[=][--](Sum m n f).
intros f a b.
unfold Sum in |- *.
unfold Sum1 in |- *.
...
Lemma inv_Sumx :
 forall (n : nat) (f : forall i : nat, i < n -> G),
 [--](Sumx _ f)[=]Sumx _ (fun (i : nat) (Hi : i < n) => [--](f i Hi)).
intro n; induction  n as [| n Hrecn].
 simpl in |- *; Algebra.
intro f; simpl in |- *.
AStepl
...
Lemma inv_Sum2 :
 forall m n : nat,
 m <= S n ->
 forall f : forall i : nat, m <= i -> i <= n -> G,
 [--](Sum2 _ _ f)[=]Sum2 _ _ (fun i Hm Hn => [--](f i Hm Hn)).
intros m n H f.
unfold Sum2 in |- *; simpl in |- *.
apply eq_symmetric_unfolded.
eapply eq_transitive_unfolded.
...
cut
 (Sum a b (fun i : nat => f i[+](fun j : nat => [--](g j)) i)[=]
...
Lemma Sumx_minus_Sumx :
 forall (n : nat) (f g : forall i : nat, i < n -> G),
 Sumx _ f[-]Sumx _ g[=]Sumx _ (fun i Hi => f i Hi[-]g i Hi).
intros n f g; unfold cg_minus in |- *.
eapply eq_transitive_unfolded.
 2: apply
     Sumx_plus_Sumx
      with (f := f) (g := fun (i : nat) (Hi : i < n) => [--](g i Hi)).
apply bin_op_wd_unfolded; Algebra.
apply inv_Sumx.
Qed.

Lemma Sum2_minus_Sum2 :
 forall m n : nat,
 m <= S n ->
 forall f g : forall i : nat, m <= i -> i <= n -> G,
 Sum2 _ _ f[-]Sum2 _ _ g[=]Sum2 _ _ (fun i Hm Hn => f i Hm Hn[-]g i Hm Hn).
...
eapply eq_transitive_unfolded.
 2: apply
     Sum2_plus_Sum2
      with
        (f := f)
        (g := fun (i : nat) (Hm : m <= i) (Hn : i <= n) => [--](g i Hm Hn));
     assumption.
apply bin_op_wd_unfolded.
 Algebra.
...
Lemma Sum_zero :
 forall (f : nat -> G) (m n : nat),
 m <= S n ->
 (forall i : nat, m <= i -> i <= n -> f i[=]Zero) -> Sum m n f[=]Zero.
intros a k l H H0. induction  l as [| l Hrecl].
...
Lemma Sum_term :
 forall (f : nat -> G) (m i n : nat),
 m <= i ->
 i <= n ->
 (forall j : nat, m <= j -> j <> i -> j <= n -> f j[=]Zero) ->
 Sum m n f[=]f i.
...
Lemma Sum0_shift :
 forall (f g : nat -> G) (n : nat),
 (forall i : nat, f i[=]g (S i)) -> g 0[+]Sum0 n f[=]Sum0 (S n) g.
...
Lemma Sum_shift :
 forall (f g : nat -> G) (m n : nat),
 (forall i : nat, f i[=]g (S i)) -> Sum m n f[=]Sum (S m) (S n) g.
...
Lemma Sumx_Sum0 :
 forall n f g, (forall i Hi, f i Hi[=]g i) -> Sumx n f[=]Sum0 n g.
...
Lemma Mengolli_Sum :
 forall (n : nat) (f : forall i : nat, i <= n -> G)
   (g : forall i : nat, i < n -> G),
 nat_less_n_fun' _ _ f ->
 (forall (i : nat) (H : i < n), g i H[=]f (S i) H[-]f i (lt_le_weak _ _ H)) ->
...
apply
 eq_transitive_unfolded
  with
    (f _ (le_n (S n))[-]f _ (le_S _ _ (le_n n))[+]
 ...
 set (f' := fun (i : nat) (H : i <= n) => f i (le_S _ _ H)) in *.
 set (g' := fun (i : nat) (H : i < n) => g i (lt_S _ _ H)) in *.
...
AStepr
 (f (S n) (le_n (S n))[+]
  ([--](f n (le_S _ _ (le_n n)))[+]f n (le_S _ _ (le_n n)))[-]
  f 0 (le_O_n (S n))).
Step_final
 (f (S n) (le_n (S n))[+][--](f n (le_S _ _ (le_n n)))[+]
  f n (le_S _ _ (le_n n))[-]f 0 (le_O_n (S n))).
Qed.

Lemma Mengolli_Sum_gen :
 forall f g : nat -> G,
 (forall n : nat, g n[=]f (S n)[-]f n) ->
 forall m n : nat, m <= S n -> Sum m n g[=]f (S n)[-]f m.
...
Lemma str_Mengolli_Sum_gen :
 forall (f g : nat -> G) (m n : nat),
 m <= S n ->
 (forall i : nat, m <= i -> i <= n -> g i[=]f (S i)[-]f i) ->
 Sum m n g[=]f (S n)[-]f m.
intros f g m n H H0.
...
Lemma Sumx_to_Sum :
 forall n : nat,
 0 < n ->
 forall f : forall i : nat, i < n -> G,
 nat_less_n_fun _ _ f -> Sumx f[=]Sum 0 (pred n) (part_tot_nat_fun _ _ f).
