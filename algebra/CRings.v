...
(** printing [*][--] %\ensuremath\times-% #&times;&minus;# *)
(** printing {*}{--} %\ensuremath\times-% #&times;&minus;# *)
...
*)
Definition distributive (S : CSetoid) (mult plus : CSetoid_bin_op S) :=
  forall x y z : S, mult x (plus y z)[=]plus (mult x y) (mult x z).

Implicit Arguments distributive [S].

Record is_CRing (G : CAbGroup) (one : G) (mult : CSetoid_bin_op G) :
  CProp := 
...
(* Syntax is discontinued Syntax constr level 1 :
                           cr_one_constant
                           [<<(APPLIST (CONST <CRings#2>. cr_one) $e0)>>] ->
                           [ "One" ] *)
(* end hide *)
(* End_SpecReals *)
...
Lemma mult_mon :
 is_CMonoid (Build_CSemiGroup R cr_mult CRing_axioms.mult_assoc) One.
elim (cr_proof R).
...
Lemma mult_wd :
 forall x1 x2 y1 y2 : R, x1[=]x2 -> y1[=]y2 -> x1[*]y1[=]x2[*]y2.
intros; Algebra.
Qed.

Lemma mult_wd_lft : forall x1 x2 y : R, x1[=]x2 -> x1[*]y[=]x2[*]y.
intros; Algebra.
Qed.

Lemma mult_wd_rht : forall x y1 y2 : R, y1[=]y2 -> x[*]y1[=]x[*]y2.
intros; Algebra.
Qed.
...
Definition is_zero_rht (S : CSetoid) (op : CSetoid_bin_op S) 
  (zero : S) : Prop := forall x : S, op x zero[=]zero.

Definition is_zero_lft (S : CSetoid) (op : CSetoid_bin_op S) 
  (zero : S) : Prop := forall x : S, op zero x[=]zero.
...
Lemma cring_mult_ap_zero : forall x y : R, x[*]y[#]Zero -> x[#]Zero.
intros x y H.
elim (bin_op_strext _ cr_mult x Zero y y).
  auto.
...
Lemma nexp_well_def : forall n : nat, fun_well_def (nexp n).
intro n; induction  n as [| n Hrecn]; red in |- *; intros; simpl in |- *;
 Algebra.
Qed.

Lemma nexp_strong_ext : forall n : nat, fun_strong_ext (nexp n).
intro n; red in |- *; induction  n as [| n Hrecn]; simpl in |- *;
 intros x y H.
 elim (ap_irreflexive _ _ H).
elim (bin_op_strext _ cr_mult _ _ _ _ H); auto.
Qed.

Definition nexp_op (n : nat) :=
  Build_CSetoid_un_op R (nexp n) (nexp_strong_ext n).
...
(* begin hide *)
(* Syntax is discontinued Syntax constr level 2 :
                           nexp_op_infix
                           [<<(APPLIST (CONST <CRings#2>. nexp_op)
                                 (EXPL 1 (ISEVAR )) $e1 $e2)>>] -> [ [<hov
                           1>$e2 [0 1] "[^]" $e1] ] *)
(* end hide *)
...
Lemma nring_comm_plus : forall n m : nat, nring (n + m)[=]nring n[+]nring m.
...
Lemma nring_comm_mult : forall n m : nat, nring (n * m)[=]nring n[*]nring m.
intros n m; induction  n as [| n Hrecn]; simpl in |- *.
 Algebra.
...
(* Syntax is discontinued Syntax constr level 1 :
                           cr_two_constant
                           [<<(APPLIST (CONST <CRings#2>. nring)
                                 (EXPL 1 (ISEVAR ))
                                 (APPLIST
                                    (MUTCONSTRUCT Coq.Init.Datatypes. nat 0 2)
                                    (APPLIST
                                       (MUTCONSTRUCT Coq.Init.Datatypes. nat 0
                                          2)
                                       (MUTCONSTRUCT Coq.Init.Datatypes. nat 0
                                          1))))>>] -> [ "Two" ] *)
(* end hide *)
                           ...
                           [<<(APPLIST (CONST <CRings#2>. nring)
                                 ...
                                 (APPLIST
                                    ...
                                    (APPLIST
                                       ...
                                       (APPLIST
                                          (MUTCONSTRUCT
                                             Coq.Init.Datatypes. nat 0 2)
                                          (MUTCONSTRUCT
                                             Coq.Init.Datatypes. nat 0 1)))))>>]
                           -> [ "Three" ] *)
(* end hide *)
...
Lemma nring_different :
 forall R : CRing,
 Char0 R -> forall i j : nat, i <> j -> nring i[#](nring j:R).
intros R H i j H0.
...
Definition zring_old (k : Z) : R :=
  caseZ_diff k (fun m n : nat => nring m[-]nring n).
...
Lemma zring_old_diff :
 forall m n : nat, zring_old (m - n)[=]nring m[-]nring n.
unfold zring_old in |- *.
intros m n.
(* Rewrite with [=] *)
apply
...
Lemma zring_old_plus :
 forall i j : Z, zring_old (i + j)[=]zring_old i[+]zring_old j.
intros i j.
pattern i in |- *.
apply diff_Z_ind.
intros m n.
...
Lemma zring_old_inv : forall i : Z, zring_old (- i)[=][--](zring_old i).
intro i.
pattern i in |- *.
apply diff_Z_ind.
intros m n.
...
Lemma zring_old_inv_one : forall x : R, zring_old (-1)[*]x[=][--]x.
intro x.
simpl in |- *.
AStepl ([--](Zero[+]One)[*]x).
AStepl ([--]One[*]x).
...
(** The [zring] function can also be defined directly.  This is done here.
*)
...
Lemma pring_aux_lemma :
 forall (p : positive) (r r' : R), r[=]r' -> pring_aux p r[=]pring_aux p r'.
simple induction p; simpl in |- *; Algebra.
Qed.

Lemma double_nring :
 forall n : nat, Two[*]nring (R:=R) n[=]nring (R:=R) (n + n).
intros.
...
Lemma pring_aux_nring :
 forall (p : positive) (n : nat),
 pring_aux p (nring n)[=]nring (Pmult_nat p n).
...
Lemma pring_convert : forall p : positive, pring p[=]nring (nat_of_P p).
...
Lemma zring_plus : forall i j : Z, zring (i + j)[=]zring i[+]zring j.
intros.
AStepl (zring_old (i + j)).
Step_final (zring_old i[+]zring_old j).
Qed.

Lemma zring_inv : forall i : Z, zring (- i)[=][--](zring i).
intro i.
AStepl (zring_old (- i)).
Step_final ([--](zring_old i)).
Qed.

Lemma zring_minus : forall i j : Z, zring (i - j)[=]zring i[-]zring j.
intros i j.
AStepl (zring_old (i - j)).
Step_final (zring_old i[-]zring_old j).
Qed.

Lemma zring_mult : forall i j : Z, zring (i * j)[=]zring i[*]zring j.
intros i j.
AStepl (zring_old (i * j)).
Step_final (zring_old i[*]zring_old j).
Qed.
...
Lemma zring_inv_one : forall x : R, zring (-1)[*]x[=][--]x.
intro x.
simpl in |- *.
unfold pring in |- *.
simpl in |- *.
...
(**
Here's an alternative def of [List_Sum_from_upto], with a lemma
 that it's equivalent to the original.
*)

(*
Fixpoint List_End [l:(list R); n:nat]:(list R) := Cases n of
    O     =>  l
...
Lemma mult_distr_sum0_lft :
 forall (f : nat -> R) (x : R) (n : nat),
 Sum0 n (fun i : nat => x[*]f i)[=]x[*]Sum0 n f.
intros f x n.
...
Lemma mult_distr_sum_lft :
 forall (f : nat -> R) (x : R) (m n : nat),
 Sum m n (fun i : nat => x[*]f i)[=]x[*]Sum m n f.
intros f x m n.
unfold Sum in |- *.
unfold Sum1 in |- *.
...
Lemma mult_distr_sum_rht :
 forall (f : nat -> R) (x : R) (m n : nat),
 Sum m n (fun i : nat => f i[*]x)[=]Sum m n f[*]x.
intros f x m n.
...
Lemma sumx_const :
 forall (n : nat) (x : R),
 Sumx (fun (i : nat) (_ : i < n) => x)[=]nring n[*]x.
...
Lemma nexp_wd : forall (x y : R) (n : nat), x[=]y -> x[^]n[=]y[^]n.
Algebra.
Qed.

Lemma nexp_strext : forall (x y : R) (n : nat), x[^]n[#]y[^]n -> x[#]y.
intros x y n H.
exact (un_op_strext_unfolded _ _ _ _ H).
Qed.

Lemma nexp_Sn : forall (x : R) (n : nat), x[*]x[^]n[=]x[^]S n.
intros x n.
Step_final (x[^]n[*]x).
Qed.
...
Lemma nexp_plus : forall (x : R) (m n : nat), x[^]m[*]x[^]n[=]x[^](m + n).
intros x m n.
...
Lemma mult_nexp : forall (x y : R) (n : nat), (x[*]y)[^]n[=]x[^]n[*]y[^]n.
intros x y n.
induction  n as [| n Hrecn].
 AStepl (One:R).
 Step_final ((One:R)[*]One).
...
Lemma nexp_mult : forall (x : R) (m n : nat), (x[^]m)[^]n[=]x[^](m * n).
intros x m n.
induction  m as [| m Hrecm].
 simpl in |- *.
 Step_final ((One:R)[^]n).
...
Lemma zero_nexp : forall (x : R) (n : nat), 0 < n -> (Zero:R)[^]n[=]Zero.
intros x n H.
induction  n as [| n Hrecn].
 inversion H.
...
Lemma inv_nexp_odd :
 forall (x : R) (n : nat), odd n -> [--]x[^]n[=][--](x[^]n).
intros x n H.
...
Lemma square_minus :
 forall x y : R, (x[-]y)[^]2[=]x[^]2[+]y[^]2[-]Two[*]x[*]y.
intros x y.
unfold cg_minus in |- *.
...
Lemma part_function_mult_strext :
 forall (x y : R) (Hx : Conj P Q x) (Hy : Conj P Q y),
 F x (prj1 R _ _ _ Hx)[*]G x (prj2 R _ _ _ Hx)[#]
 F y (prj1 R _ _ _ Hy)[*]G y (prj2 R _ _ _ Hy) -> x[#]y.
intros x y Hx Hy H.
...
Definition Fmult :=
  Build_PartFunct R _ (conj_wd _ _ _ (dom_wd _ F) (dom_wd _ G))
    (fun (x : R) (Hx : Conj P Q x) =>
     F x (prj1 R _ _ _ Hx)[*]G x (prj2 R _ _ _ Hx)) part_function_mult_strext.
...
Lemma part_function_nth_strext :
 forall (x y : R) (Hx : P x) (Hy : P y), F x Hx[^]n[#]F y Hy[^]n -> x[#]y.
intros x y Hx Hy H.
...
Definition Fnth :=
  Build_PartFunct R _ (dom_wd R F) (fun (x : R) (Hx : P x) => F x Hx[^]n)
