...
- [(cpoly_linear c p)] is $c + X * p$ #c + X * p#

*)

Inductive cpoly : Type :=
  | cpoly_zero : cpoly
  | cpoly_linear : forall (s : CR) (c : cpoly), cpoly.
...
Lemma Ccpoly_double_ind0 :
 forall P : cpoly -> cpoly -> CProp,
 (forall p : cpoly, P p cpoly_zero) ->
 (forall p : cpoly, P cpoly_zero p) ->
 (forall (p q : cpoly) (c d : CR),
  P p q -> P (cpoly_linear c p) (cpoly_linear d q)) ->
 forall p q : cpoly, P p q.
simple induction p; auto.
simple induction q; auto.
Qed.

Lemma Ccpoly_double_sym_ind0 :
 forall P : cpoly -> cpoly -> CProp,
 Csymmetric P ->
 (forall p : cpoly, P p cpoly_zero) ->
 (forall (p q : cpoly) (c d : CR),
...
Lemma Ccpoly_double_ind0' :
 forall P : cpoly -> cpoly -> CProp,
 (forall p : cpoly, P cpoly_zero p) ->
 (forall (p : cpoly) (c : CR), P (cpoly_linear c p) cpoly_zero) ->
 (forall (p q : cpoly) (c d : CR),
  P p q -> P (cpoly_linear c p) (cpoly_linear d q)) ->
 forall p q : cpoly, P p q.
simple induction p; auto.
...
Lemma cpoly_double_ind0 :
 forall P : cpoly -> cpoly -> Prop,
 (forall p : cpoly, P p cpoly_zero) ->
 (forall p : cpoly, P cpoly_zero p) ->
 (forall (p q : cpoly) (c d : CR),
  P p q -> P (cpoly_linear c p) (cpoly_linear d q)) ->
...
Lemma cpoly_eq_p_zero :
 forall p : cpoly, cpoly_eq p cpoly_zero = cpoly_eq_zero p.
simple induction p; auto.
Qed.

(**
The annoying thing about [cpoly_eq_p_zero] is that this lemma cannot be
used with the [Rewrite] tactic when the goal lives in [CProp]
(instead of [Prop]). This is because [==] gives a [Prop], and
Coq's type system does not allow [Prop] elimination over a [CProp].

The remedy would be to introduce a [CProp]-valued equality, say
[CProp_eq].
Unfortunately, the [Rewrite]-tactic does not work for [CProp_eq].
Therefore we are forced to introduce two lemmas stating the implications
back and forth.
*)

Lemma cpoly_eq_p_zero_ :
 forall p : cpoly, cpoly_eq p cpoly_zero -> cpoly_eq_zero p.
simple induction p; auto.
Qed.

Lemma _cpoly_eq_p_zero :
 forall p : cpoly, cpoly_eq_zero p -> cpoly_eq p cpoly_zero.
simple induction p; auto.
Qed.
...
Lemma cpoly_ap_p_zero :
 forall p : cpoly, cpoly_ap_zero p = cpoly_ap p cpoly_zero.
simple induction p; auto.
Qed.
...
Lemma cpoly_ap_p_zero_ :
 forall p : cpoly, cpoly_ap p cpoly_zero -> cpoly_ap_zero p.
simple induction p; auto.
Qed.
...
apply Ccpoly_double_sym_ind0.
 ...
 elim H1; intro H2.
  ...
  elim H3; auto.
 generalize (_cpoly_ap_p_zero _ H2); clear H2; intro H2.
 elim (H0 H2); auto.
...
Let cpoly_linear_cs (c : CR) (p : cpoly_csetoid) : cpoly_csetoid :=
  cpoly_linear c p.

Lemma Ccpoly_ind_cs :
 forall P : cpoly_csetoid -> CProp,
 P cpoly_zero_cs ->
 (forall (p : cpoly_csetoid) (c : CR), P p -> P (cpoly_linear_cs c p)) ->
 forall p : cpoly_csetoid, P p.
simple induction p; auto.
unfold cpoly_linear_cs in X0.
auto.
Qed.

Lemma Ccpoly_double_ind0_cs :
 forall P : cpoly_csetoid -> cpoly_csetoid -> CProp,
 (forall p : cpoly_csetoid, P p cpoly_zero_cs) ->
 (forall p : cpoly_csetoid, P cpoly_zero_cs p) ->
 (forall (p q : cpoly_csetoid) (c d : CR),
  P p q -> P (cpoly_linear_cs c p) (cpoly_linear_cs d q)) ->
 forall p q : cpoly_csetoid, P p q.
simple induction p.
auto.
simple induction q.
auto.
...
Lemma Ccpoly_double_sym_ind0_cs :
 forall P : cpoly_csetoid -> cpoly_csetoid -> CProp,
 Csymmetric P ->
 (forall p : cpoly_csetoid, P p cpoly_zero_cs) ->
 (forall (p q : cpoly_csetoid) (c d : CR),
...
Lemma cpoly_double_ind0_cs :
 forall P : cpoly_csetoid -> cpoly_csetoid -> Prop,
 (forall p : cpoly_csetoid, P p cpoly_zero_cs) ->
 (forall p : cpoly_csetoid, P cpoly_zero_cs p) ->
 (forall (p q : cpoly_csetoid) (c d : CR),
  P p q -> P (cpoly_linear_cs c p) (cpoly_linear_cs d q)) ->
...
Lemma cpoly_double_sym_ind0_cs :
 forall P : cpoly_csetoid -> cpoly_csetoid -> Prop,
 Tsymmetric P ->
 (forall p : cpoly_csetoid, P p cpoly_zero_cs) ->
 (forall (p q : cpoly_csetoid) (c d : CR),
...
Lemma cpoly_lin_eq_zero_ :
 forall (p : cpoly_csetoid) (c : CR),
...
Lemma cpoly_linear_strext : bin_fun_strong_ext _ _ _ cpoly_linear_cs.
...
Lemma cpoly_linear_wd : bin_fun_well_def _ _ _ cpoly_linear_cs.
apply bin_fun_strong_ext_imp_well_def.
exact cpoly_linear_strext.
Qed.

Definition cpoly_linear_fun :=
  Build_CSetoid_bin_fun _ _ _ _ cpoly_linear_strext.

Lemma Ccpoly_double_comp_ind :
 forall P : cpoly_csetoid -> cpoly_csetoid -> CProp,
 (forall p1 p2 q1 q2 : cpoly_csetoid,
  p1[=]p2 -> q1[=]q2 -> P p1 q1 -> P p2 q2) ->
...
Lemma cpoly_zero_plus :
 forall p : cpoly_csetoid, cpoly_plus_cs cpoly_zero_cs p = p.
auto.
Qed.

Lemma cpoly_plus_zero :
 forall p : cpoly_csetoid, cpoly_plus_cs p cpoly_zero_cs = p.
simple induction p.
auto.
auto.
Qed.

Lemma cpoly_lin_plus_lin :
 forall (p q : cpoly_csetoid) (c d : CR),
...
Lemma cpoly_plus_commutative :
 forall p q : cpoly_csetoid, cpoly_plus_cs p q[=]cpoly_plus_cs q p.
intros.
pattern p, q in |- *.
...
Lemma cpoly_plus_q_ap_q :
 forall p q : cpoly_csetoid, cpoly_plus_cs p q[#]q -> p[#]cpoly_zero_cs.
...
Lemma cpoly_p_plus_ap_p :
 forall p q : cpoly_csetoid, cpoly_plus_cs p q[#]p -> q[#]cpoly_zero.
intros.
...
Lemma cpoly_ap_zero_plus :
 forall p q : cpoly_csetoid,
...
cut (c[+]d[#]Zero[+]Zero).
intro H2.
elim (bin_op_strext _ _ _ _ _ _ H2); intro H3.
left.
simpl in |- *.
left.
assumption.
...
cut (c[+]c0[#]Zero[+]d).
intro H3.
elim (bin_op_strext _ _ _ _ _ _ H3).
intro H4.
left.
apply _cpoly_lin_ap_zero.
auto.
...
cut (c[+]c0[#]d[+]Zero).
intro H4.
elim (bin_op_strext _ _ _ _ _ _ H4).
intro.
left.
apply _cpoly_lin_ap_lin.
auto.
...
intros.
elim X1; intro H2.
elim (bin_op_strext _ _ _ _ _ _ H2); auto.
intro.
left.
left; auto.
intro. right.
...
Definition cpoly_csemi_grp :=
  Build_CSemiGroup cpoly_csetoid cpoly_plus_op cpoly_plus_associative.
...
Lemma cpoly_zero_mult_cr :
 forall c : CR, cpoly_mult_cr_cs cpoly_zero_cs c = cpoly_zero_cs.
auto.
Qed.

Lemma cpoly_lin_mult_cr :
 forall (c d : CR) (q : cpoly_csetoid),
 cpoly_mult_cr_cs (cpoly_linear_cs d q) c =
 cpoly_linear_cs (c[*]d) (cpoly_mult_cr_cs q c).
auto.
Qed.

Lemma cpoly_mult_cr_zero :
 forall p : cpoly_csetoid, cpoly_mult_cr_cs p Zero[=]cpoly_zero_cs.
...
elim X0; try auto.
elim H0; intro H1.
generalize (bin_op_strext _ _ _ _ _ _ H1); tauto.
elim X; auto.
Qed.
...
Lemma cpoly_zero_mult :
 forall q : cpoly_csetoid, cpoly_mult_cs cpoly_zero_cs q = cpoly_zero_cs.
auto.
Qed.

Lemma cpoly_lin_mult :
 forall (c : CR) (p q : cpoly_csetoid),
...
Lemma cpoly_mult_cr_dist :
 forall (p q : cpoly_csetoid) (c : CR),
...
Lemma cpoly_mult_cr_assoc_mult_cr :
 forall (p : cpoly_csetoid) (c d : CR),
...
Lemma cpoly_mult_cr_assoc_mult :
 forall (p q : cpoly_csetoid) (c : CR),
...
Lemma cpoly_mult_zero :
 forall p : cpoly_csetoid, cpoly_mult_cs p cpoly_zero_cs[=]cpoly_zero_cs.
intros.
pattern p in |- *; apply cpoly_ind_cs.
Algebra.
intros.
...
Lemma cpoly_mult_cr_one :
 forall p : cpoly_csetoid, cpoly_mult_cr_cs p One[=]p.
intro.
pattern p in |- *; apply cpoly_ind_cs.
Algebra.
intros.
...
Lemma cpoly_one_mult :
 forall p : cpoly_csetoid, cpoly_mult_cs cpoly_one p[=]p.
intro.
unfold cpoly_one in |- *.
...
Lemma cpoly_mult_one :
 forall p : cpoly_csetoid, cpoly_mult_cs p cpoly_one[=]p.
intro.
...
Lemma cpoly_mult_monoid :
 is_CMonoid (Build_CSemiGroup cpoly_csetoid cpoly_mult_op cpoly_mult_assoc)
   cpoly_one.
apply Build_is_CMonoid.
...
Definition cpoly_cring : CRing :=
  Build_CRing cpoly_cabgroup cpoly_one cpoly_mult_op cpoly_is_CRing.

Lemma cpoly_constant_strext :
 fun_strong_ext (S1:=CR) (S2:=cpoly_cring) cpoly_constant.
unfold fun_strong_ext in |- *.
unfold cpoly_constant in |- *.
simpl in |- *.
intros x y H.
...
Lemma cpoly_constant_wd :
 fun_well_def (S1:=CR) (S2:=cpoly_cring) cpoly_constant.
apply fun_strong_ext_imp_well_def.
exact cpoly_constant_strext.
Qed.

Definition _C_ :=
  Build_CSetoid_fun CR cpoly_cring cpoly_constant cpoly_constant_strext.

Definition _X_ : cpoly_cring := cpoly_linear_cs Zero (One:cpoly_cring).

Definition cpoly_x_minus_c (c : CR) : cpoly_cring :=
  cpoly_linear_cs [--]c (One:cpoly_cring).

Lemma cpoly_x_minus_c_strext :
 fun_strong_ext (S1:=CR) (S2:=cpoly_cring) cpoly_x_minus_c.
unfold fun_strong_ext in |- *.
unfold cpoly_x_minus_c in |- *.
simpl in |- *.
intros x y H.
elim H; intro H0.
apply (un_op_strext _ _ _ _ H0).
elim H0; intro H1.
elim (ap_irreflexive_unfolded _ _ H1).
elim H1.
Qed.

Lemma cpoly_x_minus_c_wd :
 fun_well_def (S1:=CR) (S2:=cpoly_cring) cpoly_x_minus_c.
apply fun_strong_ext_imp_well_def.
...
Lemma Ccpoly_induc :
 forall P : RX -> CProp,
 P Zero ->
 (forall (p : RX) (c : CR), P p -> P (c[+X*]p)) -> forall p : RX, P p.
exact (Ccpoly_ind_cs CR).
Qed.

Lemma Ccpoly_double_sym_ind :
 forall P : RX -> RX -> CProp,
 Csymmetric P ->
 (forall p : RX, P p Zero) ->
 (forall (p q : RX) (c d : CR), P p q -> P (c[+X*]p) (d[+X*]q)) ->
 forall p q : RX, P p q.
...
Lemma Cpoly_double_comp_ind :
 forall P : RX -> RX -> CProp,
 (forall p1 p2 q1 q2 : RX, p1[=]p2 -> q1[=]q2 -> P p1 q1 -> P p2 q2) ->
 P Zero Zero ->
 (forall (p q : RX) (c d : CR), P p q -> P (c[+X*]p) (d[+X*]q)) ->
 forall p q : RX, P p q.
...
Lemma Cpoly_triple_comp_ind :
 forall P : RX -> RX -> RX -> CProp,
 (forall p1 p2 q1 q2 r1 r2 : RX,
  p1[=]p2 -> q1[=]q2 -> r1[=]r2 -> P p1 q1 r1 -> P p2 q2 r2) ->
 P Zero Zero Zero ->
 (forall (p q r : RX) (c d e : CR),
  P p q r -> P (c[+X*]p) (d[+X*]q) (e[+X*]r)) -> forall p q r : RX, P p q r.
...
Lemma cpoly_apply_strext : bin_fun_strong_ext _ _ _ cpoly_apply.
unfold bin_fun_strong_ext in |- *.
do 2 intro.
pattern x1, x2 in |- *.
...
intro.
elim (bin_op_strext _ _ _ _ _ _ X1); intro H2.
left.
...
simpl in |- *.
intros.
elim (bin_op_strext _ _ _ _ _ _ X0); intro H1.
auto.
elim (bin_op_strext _ _ _ _ _ _ H1); intro H2.
auto.
elim (X _ _ H2); auto.
Qed.

Lemma cpoly_apply_wd : bin_fun_well_def _ _ _ cpoly_apply.
apply bin_fun_strong_ext_imp_well_def.
exact cpoly_apply_strext.
Qed.

Definition cpoly_apply_fun :=
  Build_CSetoid_bin_fun _ _ _ _ cpoly_apply_strext.
...
(* SUPERFLUOUS *)
Lemma poly_linear :
 forall (c : R) (f : RX), (cpoly_linear _ c f:RX)[=]_X_[*]f[+]_C_ c.
intros.
AStepr (_C_ c[+]_X_[*]f).
exact (cpoly_lin f c).
Qed.
...
Lemma _c_mult_lin :
 forall (p : RX) (c d : R), _C_ c[*](d[+X*]p)[=]c[*]d[+X*]_C_ c[*]p.
intros.
pattern p in |- *.
apply cpoly_induc.
simpl in |- *.
...
(* SUPERFLUOUS ? *)
Lemma lin_mult :
 forall (p q : RX) (c : R), (c[+X*]p)[*]q[=]_C_ c[*]q[+]_X_[*](p[*]q).
intros.
...
Lemma poly_eq_zero :
 forall p : RX, p[=]cpoly_zero R -> forall x : R, p ! x[=]Zero.
intros.
...
Lemma apply_wd :
 forall (p p' : RX) (x x' : R), p[=]p' -> x[=]x' -> p ! x[=]p' ! x'.
intros.
Algebra.
Qed.

Lemma cpolyap_pres_eq : forall (f : RX) (x y : R), x[=]y -> f ! x[=]f ! y.
intros.
Algebra.
Qed.

Lemma cpolyap_strext : forall (f : RX) (x y : R), f ! x[#]f ! y -> x[#]y.
intros f x y H.
...
Definition cpoly_csetoid_op (f : RX) : CSetoid_un_op R :=
  Build_CSetoid_fun _ _ (fun x : R => f ! x) (cpolyap_strext f).
...
Lemma plus_apply : forall (p q : RX) (x : R), (p[+]q) ! x[=]p ! x[+]q ! x.
intros.
...
Lemma inv_apply : forall (p : RX) (x : R), ([--]p) ! x[=][--]p ! x.
intros.
pattern p in |- *.
apply cpoly_induc.
simpl in |- *.
...
Lemma minus_apply : forall (p q : RX) (x : R), (p[-]q) ! x[=]p ! x[-]q ! x.
intros.
AStepl (p[+][--]q) ! x.
AStepr (p ! x[+][--]q ! x).
AStepl (p ! x[+]([--]q) ! x).
...
Lemma _c_mult_apply : forall (q : RX) (c x : R), (_C_ c[*]q) ! x[=]c[*]q ! x.
intros.
...
Lemma mult_apply : forall (p q : RX) (x : R), (p[*]q) ! x[=]p ! x[*]q ! x.
intros.
pattern p in |- *.
apply cpoly_induc.
simpl in |- *.
...
Lemma nexp_apply : forall (p : RX) (n : nat) (x : R), (p[^]n) ! x[=]p ! x[^]n.
intros.
...
Lemma poly_inv_apply :
 forall (p : RX) (x : R), (cpoly_inv _ p) ! x[=][--]p ! x.
exact inv_apply.
Qed.

Lemma Sum0_cpoly_ap :
 forall (f : nat -> RX) (a : R) (k : nat),
 (Sum0 k f) ! a[=]Sum0 k (fun i : nat => (f i) ! a).
intros.
...
Lemma Sum_cpoly_ap :
 forall (f : nat -> RX) (a : R) (k l : nat),
 (Sum k l f) ! a[=]Sum k l (fun i : nat => (f i) ! a).
unfold Sum in |- *.
unfold Sum1 in |- *.
intros.
unfold cg_minus in |- *.
...
Lemma cpoly_double_ind :
 forall P : Cpoly_cring -> Cpoly_cring -> Prop,
 (forall p : Cpoly_cring, P p Zero) ->
 (forall p : Cpoly_cring, P Zero p) ->
 (forall (p q : Cpoly_cring) (c d : CR), P p q -> P (c[+X*]p) (d[+X*]q)) ->
 forall p q : Cpoly_cring, P p q.
