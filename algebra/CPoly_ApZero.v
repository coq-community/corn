...
Definition distinct1 (A : CSetoid) (f : nat -> A) :=
  forall i j : nat, i <> j -> f i[#]f j.
...
%\begin{convention}%
Let [R] be a field, [a_ : nat->R] with
[(distinct1 a_)] and let [f] be a polynomial over [R], 
[n] a natural with [(degree_le n f)], i.e. [f] has degree at most [n].
%\end{convention}%
*)
...
Lemma poly_linear_factor :
 forall (f : cpoly_cring R) (a : R),
 f ! a[=]Zero -> {f' : cpoly_cring R | f[=](_X_[-]_C_ a)[*]f'}.
intros.
...
Lemma zero_poly :
 forall (n : nat) (f : cpoly_cring R),
 degree_le n f -> (forall i : nat, i <= n -> f ! (a_ i)[=]Zero) -> f[=]Zero.
intro.
...
Lemma identical_poly :
 forall f g : cpoly_cring R,
 degree_le n f ->
 degree_le n g ->
 (forall i : nat, i <= n -> f ! (a_ i)[=]g ! (a_ i)) -> f[=]g.
intros.
...
Lemma poly_01_factor'_degree :
 forall n : nat, degree_le 1 (poly_01_factor' n).
intros.
...
Lemma poly_01_factor'_zero :
 forall n : nat, (poly_01_factor' n) ! (a_ n)[=]Zero.
intros.
unfold poly_01_factor' in |- *.
...
Lemma poly_01_factor'_apzero :
 forall n i : nat, i <> n -> (poly_01_factor' n) ! (a_ i)[#]Zero.
intros.
unfold poly_01_factor' in |- *.
...
Definition poly_01_factor (n i : nat) (H : i <> n) :=
  poly_01_factor' n[*]
  _C_ (One[/] (poly_01_factor' n) ! (a_ i)[//]poly_01_factor'_apzero n i H).

Lemma poly_01_factor_degree :
 forall (n i : nat) (H : i <> n), degree_le 1 (poly_01_factor n i H).
intros.
unfold poly_01_factor in |- *.
replace 1 with (1 + 0).
...
Lemma poly_01_factor_zero :
 forall (n i : nat) (H : i <> n), (poly_01_factor n i H) ! (a_ n)[=]Zero.
intros.
unfold poly_01_factor in |- *.
AStepl
...
Lemma poly_01_factor_one :
 forall (n i : nat) (H : i <> n), (poly_01_factor n i H) ! (a_ i)[=]One.
intros.
unfold poly_01_factor in |- *.
AStepl
...
Lemma poly_01_degree' : forall n i : nat, degree_le (S n) (poly_01 i n).
intros.
induction  n0 as [| n0 Hrecn0]. intros.
simpl in |- *.
...
Lemma poly_01_degree : forall n i : nat, i <= n -> degree_le n (poly_01 i n).
intros.
induction  n0 as [| n0 Hrecn0]; intros.
simpl in |- *.
elim (eq_nat_dec i 0); intro y.
...
Lemma poly_01_zero :
 forall n i j : nat, j <= n -> j <> i -> (poly_01 i n) ! (a_ j)[=]Zero.
intros.
induction  n0 as [| n0 Hrecn0]; intros.
...
Lemma poly_01_one : forall n i : nat, (poly_01 i n) ! (a_ i)[=]One.
intros.
induction  n0 as [| n0 Hrecn0]; intros.
simpl in |- *.
elim (eq_nat_dec i 0); intro y.
...
Lemma poly_representation'' :
 forall (a : nat -> R) (i : nat),
 i <= n -> (forall j : nat, j <> i -> a j[=]Zero) -> Sum 0 n a[=]a i.
intro. intro.
elim i.
intros.
...
Lemma poly_representation' :
 forall (f_ : nat -> cpoly_cring R) (k : nat),
 k <= n ->
 (Sum 0 n (fun i : nat => f_ i[*]poly_01 i n)) ! (a_ k)[=](f_ k) ! (a_ k).
intros.
apply
...
Lemma poly_representation :
 f[=]Sum 0 n (fun i : nat => _C_ f ! (a_ i)[*]poly_01 i n).
...
Lemma poly_apzero :
 forall R : CField,
 Char0 R -> forall f : cpoly_cring R, f[#]Zero -> {c : R | f ! c[#]Zero}.
intros R H f H0.
...
Lemma poly_extensional :
 forall R : CField,
 Char0 R ->
 forall p q : cpoly_cring R, (forall x : R, p ! x[=]q ! x) -> p[=]q.
intros R H p q H0.
apply cg_inv_unique_2.
apply not_ap_imp_eq. unfold Not in |- *. intros H1.
elim (poly_apzero _ H (p[-]q)). intros x H2.
...
Lemma Cpoly_apzero_interval :
 forall f : cpoly_cring R,
 f[#]Zero ->
 forall a b : R, a[<]b -> {c : R | a[<=]c /\ c[<=]b | f ! c[#]Zero}.
intros f H a b H0.
