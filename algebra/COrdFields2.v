...
Lemma plus_resp_leEq_both :
 forall x y a b : R, x[<=]y -> a[<=]b -> x[+]a[<=]y[+]b.
 intros.
...
Lemma minus_resp_less_leEq :
 forall x y x' y' : R, x[<=]y -> y'[<]x' -> x[-]x'[<]y[-]y'.
intros.
AStepl (x[+][--]x').
AStepr (y[+][--]y').
...
Multiplication and division respect $\leq$#leEq#
*)

Lemma mult_resp_leEq_rht :
 forall x y z : R, x[<=]y -> Zero[<=]z -> x[*]z[<=]y[*]z.
unfold leEq in |- *.
intros.
intro H1.
...
Lemma recip_resp_leEq :
 forall (x y : R) (x_ : x[#]Zero) (y_ : y[#]Zero),
 Zero[<]y -> y[<=]x -> (One[/] x[//]x_)[<=](One[/] y[//]y_).
unfold leEq in |- *.
...
Lemma mult_cancel_leEq :
 forall x y z : R, Zero[<]z -> x[*]z[<=]y[*]z -> x[<=]y.
unfold leEq in |- *.
intros.
intro.
apply H.
...
Lemma shift_mult_leEq :
 forall x y z : R,
 Zero[<]z -> forall z_ : z[#]Zero, x[<=](y[/] z[//]z_) -> x[*]z[<=]y.
intros.
RStepr ((y[/] z[//]z_)[*]z).
...
Lemma shift_mult_leEq' :
 forall x y z : R,
 Zero[<]z -> forall z_ : z[#]Zero, x[<=](y[/] z[//]z_) -> z[*]x[<=]y.
intros.
RStepr (z[*](y[/] z[//]z_)).
...
Lemma shift_div_leEq :
 forall x y z : R,
 Zero[<]z -> forall z_ : z[#]Zero, x[<=]y[*]z -> (x[/] z[//]z_)[<=]y.
intros.
RStepr (y[*]z[/] z[//]z_).
apply div_resp_leEq.
assumption.
...
Lemma shift_div_leEq' :
 forall x y z : R,
 Zero[<]z -> forall z_ : z[#]Zero, x[<=]z[*]y -> (x[/] z[//]z_)[<=]y.
intros.
RStepr (z[*]y[/] z[//]z_).
apply div_resp_leEq.
assumption.
...
Lemma nonneg_div_three :
 forall eps : R, Zero[<=]eps -> Zero[<=]eps [/]ThreeNZ.
intros.
...
Lemma nonneg_div_three' :
 forall eps : R, Zero[<=]eps -> eps [/]ThreeNZ[<=]eps.
intros.
apply shift_div_leEq.
apply pos_three.
...
Lemma suc_leEq_dub :
 forall n : nat, nring (R:=R) (S (S n))[<=]Two[*]nring (S n).
intro n.
...
Lemma leEq_nring : forall n m : nat, nring (R:=R) n[<=]nring m -> n <= m.
...
Lemma nexp_resp_pos : forall (x : R) (k : nat), Zero[<]x -> Zero[<]x[^]k.
intros.
elim k.
simpl in |- *.
apply pos_one.
...
Lemma mult_resp_nonneg :
 forall x y : R, Zero[<=]x -> Zero[<=]y -> Zero[<=]x[*]y.
...
Lemma nexp_resp_less :
 forall (x y : R) (n : nat), 1 <= n -> Zero[<=]x -> x[<]y -> x[^]n[<]y[^]n.
intros.
induction  n as [| n Hrecn].
elimtype False.
inversion H.
...
Lemma power_cancel_leEq :
 forall (x y : R) (k : nat), 0 < k -> Zero[<=]y -> x[^]k[<=]y[^]k -> x[<=]y.
unfold leEq in |- *. intros. intro. apply H1.
apply nexp_resp_less; auto.
Qed.

Lemma power_cancel_less :
 forall (x y : R) (k : nat), Zero[<=]y -> x[^]k[<]y[^]k -> x[<]y.
intros x y k H H0.
...
Lemma Sumx_resp_leEq :
 forall (n : nat) (f g : forall i : nat, i < n -> R),
...
Lemma Sum2_resp_leEq :
 forall m n : nat,
 m <= S n ->
 forall f g : forall i : nat, m <= i -> i <= n -> R,
 (forall (i : nat) (Hm : m <= i) (Hn : i <= n), f i Hm Hn[<=]g i Hm Hn) ->
 Sum2 f[<=]Sum2 g.
intros.
unfold Sum2 in |- *.
apply Sum_resp_leEq.
...
Lemma approach_zero :
 forall x : R, (forall e : R, Zero[<]e -> x[<]e) -> Not (Zero[<]x).
 intros.
 intro.
 cut (x[<]x [/]TwoNZ).
...
Lemma mult_cancel_pos_rht :
 forall x y : R, Zero[<]x[*]y -> Zero[<=]x -> Zero[<]y.
intros x y H H0.
...
Lemma mult_cancel_pos_lft :
 forall x y : R, Zero[<]x[*]y -> Zero[<=]y -> Zero[<]x.
intros.
...
Lemma even_power_pos :
 forall n : nat, even n -> forall x : R, x[#]Zero -> Zero[<]x[^]n.
intros.
...
Lemma odd_power_cancel_pos :
 forall n : nat, odd n -> forall x : R, Zero[<]x[^]n -> Zero[<]x.
intros n H x H0.
induction  n as [| n Hrecn].
...
Lemma pos_nring_S : forall n : nat, ZeroR[<]nring (S n).
...
Lemma square_eq_pos :
 forall x a : R, Zero[<]a -> Zero[<]x -> x[^]2[=]a[^]2 -> x[=]a.
intros.
...
Lemma square_eq_neg :
 forall x a : R, Zero[<]a -> x[<]Zero -> x[^]2[=]a[^]2 -> x[=][--]a.
intros.
elim (square_eq _ x a); intros; auto.
elimtype False.
apply less_irreflexive_unfolded with (x := ZeroR).
...
Definition one_div_succ (R : COrdField) (i : nat) : R :=
  One[/] Snring R i[//]nringS_ap_zero _ i.
...
Lemma one_div_succ_resp_leEq :
 forall m n : nat, m <= n -> one_div_succ (R:=R) n[<=]one_div_succ m.
...
Lemma one_div_succ_pos : forall i : nat, (Zero:R)[<]one_div_succ i.
intro.
...
Lemma one_div_succ_resp_less :
 forall i j : nat, i < j -> one_div_succ j[<]one_div_succ (R:=R) i.
intros.
unfold one_div_succ in |- *. unfold Snring in |- *. intros.
apply recip_resp_less.
apply pos_nring_S.
