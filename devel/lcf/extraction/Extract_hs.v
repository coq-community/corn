...
Extract Constant pos_fact_ap_zero__to_realize =>
 "\n -> pring_ap_zero_opt_opt (pos_fact n)".

Extract Constant grows =>
"\x w z0 h0 hw hz -> grows_opt x w z0 (r_observer w O) (r_observer z0 O) h0 hw hz".

Extract Constant lft_rht_concrete =>
"\a b hab -> lft_rht_extract a b (r_observer a O) (r_observer b O) hab".
...
Extraction "all"
 Qplus' Qmult' Cauchy_IR.Cauchy_IR R_observer C_Re_observer C_Im_observer one_R one_C half
 sqrt_two sixteen_R Series.E E_2 E_3 E_4 pring_ap_zero_opt_opt my_fact
 my_fact_ap_zero new_E pi grows_opt R_as_CReals Q_as_COrdField Q_is_archemaedian
 sqrt_two_lcf lft_rht_extract (* Pi *) one_fta sqrt_two_fta.
