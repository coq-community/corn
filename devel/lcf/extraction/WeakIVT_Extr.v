...
Axiom lft_rht_concrete : forall a b : IR, a [<] b ->
  (Two[*]a[+]b)[/]ThreeNZ [<] (a[+]Two[*]b)[/]ThreeNZ.
...
assert (H1 : x1 [<] y1).
 unfold x1, y1 in |- *; apply lft_rht_concrete; auto.
...
apply less_leEq; apply less_transitive_unfolded with x1; unfold x1 in |- *;
 [ apply a_lft | apply lft_rht_concrete ]; auto.
...
apply less_leEq; apply less_transitive_unfolded with y1; unfold y1 in |- *;
 [ apply lft_rht_concrete | apply rht_b ]; auto.
Qed.
(* end hide *)
...
elim
 (IVT_seq_lemma (pairT IVTseq3 IVTseq4) (pairT IVTH3 IVTH4) IVTprf0
    (conj IVTz3 IVTz4)).
