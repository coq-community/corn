...
Fixpoint beq (n m:nat) {struct n}:bool:=
match n, m with
   | O, O => true
   | S n', O => false
   | O, S m' => false
...
Fixpoint bleq (n m:nat) {struct n} : bool :=
   match n, m with
   | O, _ => true
   | S n', O => false
   | S n', S m' => (bleq n' m')
...
Definition blt (n m:nat):= (bleq (S n) m).
...
Lemma blte_eq_lte : forall n m, n-<m<->n<=m.
intros n m.
split.
...
Lemma bleq_imp_lte : forall n m, ((n-<m) = true) ->n<=m.
intros.
elim  (blte_eq_lte n m).
intros.
apply H0.
trivial.
Qed.

Lemma lte_imp_bleq : forall n m, n<=m -> ((n-<m) = true).
intros.
elim  (blte_eq_lte n m).
intros.
apply H1.
...
Hint Resolve lte_imp_bleq lt_imp_blt eq_imp_beq:pbdb.
Hint Resolve bleq_imp_lte blt_imp_lt beq_imp_eq:bpdb.

Lemma disjlemmabp:forall a b: bool,   ((orb a b) = true)-> forall A B  : Prop, (a = true -> A)->(b = true -> B )-> (A \/ B).
intros a b.
elim a.
intros H A B H1 H2.
left.
...
Lemma disjlemmapb:forall A B : Prop, (A \/ B) -> forall a b : bool, (A-> a = true)->(B -> b = true)->  (orb a b) = true.
intros A B H a b.
elim H.
intros.
rewrite (H1 H0).
...
Lemma conjlemmabp:forall a b: bool,   ((andb a b) = true)-> forall A B  : Prop, (a = true -> A)->(b = true -> B )-> (A /\ B).
intros a b.
elim b; elim a.
intros H A B H1 H2.
split.
...
Lemma conjlemmapb:forall A B : Prop, (A /\ B) -> forall a b : bool, (A-> a = true)->(B -> b = true)->  (andb a b) = true.
intros A B H a b.
elim H.
intros.
rewrite (H2 H0).
...
Lemma implemmabp:forall a b: bool,   ((implb a b) = true)-> forall A B  : Prop, (A -> a = true)->(b = true -> B)-> (A -> B).
intros a b.
elim b; elim a.
intros _ A B _  H2 _.
apply H2; trivial.
...
Lemma implemmapb:forall A B : Prop, (A ->  B) -> forall a b : bool, (a = true -> A)->(B -> b= true)->  (implb a b) = true.
intros A B H a b.
elim a; elim b.
intros.
simpl; trivial.
...
Lemma implemma:forall A B A' B': Prop, (A ->  B) -> (A' -> A)->(B -> B')->  (A' -> B').
intros.
apply H1.
apply H.
apply H0.
...
Lemma notlemmabp:forall a : bool, negb a = true -> forall A : Prop, (A -> a = true)-> ~A.
intro a.
elim a.
intro H; inversion H.
intros _ A H pa.
...
Lemma notlemmapb:forall A : Prop, ~A -> forall a : bool, (a = true -> A)-> (negb a ) = true.
intros A H a.
elim a.
intros.
...
Lemma notlemma:forall A A' : Prop, ~A -> (A' -> A)-> (~ A' ).
intros.
intro.
apply H.
apply H0.
...
Ltac preBoolToPropSolve H := 
   match goal with 
     ...
     | |- (?b1 \/ ?b2) => 
            (elim H; 
              [let H1 := fresh "H" in (intro H1; preBoolToPropSolve H1) 
              |let H2 := fresh "H" in (intro H2; preBoolToPropSolve H2)])
     | |-  (?b1 -> ?b2) => 
