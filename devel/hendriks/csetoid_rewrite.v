...
Require Export csetoid_rewrite_prelim.

Implicit Types S T : CSetoid.

Section syntactic_total_setoid_expressions.

(** 
Syntactic setoid expressions reflecting setoid terms 
built from total setoid functions. 
[S] is the type (a csetoid) of variables, 
i.e. the subterm that is to be replaced later on. *)

Inductive tot_set_exp S : CSetoid -> Type :=
...
Fixpoint tse_int S T (r:S) (e:(tot_set_exp S T)) {struct e} : T :=
  match e with
  | tse_var => r
...
Lemma tse_int_wd :
  ...
  r1 [=] r2 
  -> forall e:(tot_set_exp S T), 
  tse_int S T r1 e [=] tse_int S T r2 e.
Proof.
intros S T r1 r2 h.
...
Ltac tse_quote S T r t :=
  ...
  | (csbf_fun ?T1 ?T2 ?T3 ?f ?t1 ?t2) =>
      let e1 := tse_quote S T1 r t1 
      with e2 := tse_quote S T2 r t2 in
      constr:(tse_bfun S T1 T2 T3 f e1 e2)
  | ?t0 => constr:(tse_con S T t0)
  end.

(** Given [S:CSetoid;r1,r2:S] and [A:Prop] or [A:CProp], 
[(tot_repl_in_form S r1 r2 A)] 
...
Ltac tot_repl_in_form S r1 r2 A :=
  ...
  | (csp_pred ?T ?P ?t) =>
      ...
      let u := constr:(tse_int S T r2 e) in
      constr:(P u)
  | (csr_rel ?T ?R ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let u1 := constr:(tse_int S T r2 e1)
      with u2 := constr:(tse_int S T r2 e2) in
      constr:(R u1 u2)
  | (Ccsr_rel ?T ?R ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
  ...
  | (cs_eq (c:=?T) ?t1 ?t2) =>
      ...
      let u1 := constr:(tse_int S T r2 e1)
      with u2 := constr:(tse_int S T r2 e2) in
      constr:(cs_eq (c:=T) u1 u2)
  | (cs_ap (c:=?T) ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let u1 := constr:(tse_int S T r2 e1)
      with u2 := constr:(tse_int S T r2 e2) in
      constr:(cs_ap (c:=T) u1 u2)
  | (?A1 -> ?A2) =>
...
[(tot_set_rewr_prf2 S r1 r2 h h0 A) : A[r1:=r2] -> A] 
where [A[r1:=r2]] denotes [(tot_repl_in_form S r1 r2 A)]. 
The argument [h0:r2[=]r1] is present to avoid iterated application 
of [eq_symmetric].
*)
...
Ltac tot_set_rewr_prf1 S r1 r2 H H0 A :=
  ...
  | (Ccsr_rel ?T ?R ?t1 ?t2) =>
      ...
      with d1 := constr:(tse_int_wd S T r1 r2 H e1)
      with d2 := constr:(tse_int_wd S T r1 r2 H e2) in
      constr:(fun a:(R s1 s2) =>  (Ccsr_wd T R s1 s2 u1 u2 a d1 d2))
  | (cs_eq (c:=?T) ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let s1 := constr:(tse_int S T r1 e1)
      ...
      with d1 := constr:(tse_int_wd S T r1 r2 H e1)
      with d2 := constr:(tse_int_wd S T r1 r2 H e2) in
      constr:(fun a:(cs_eq (c:=T) s1 s2) => (eq_wd T s1 s2 u1 u2 a d1 d2))
  | (cs_ap (c:=?T) ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let s1 := constr:(tse_int S T r1 e1)
      ...
      with d1 := constr:(tse_int_wd S T r1 r2 H e1)
      with d2 := constr:(tse_int_wd S T r1 r2 H e2) in
      constr:(fun a:(cs_ap (c:=T) s1 s2) => (ap_wd T s1 s2 u1 u2 a d1 d2))
  | (?A1 -> ?A2) =>
 ...
 with tot_set_rewr_prf2 S r1 r2 H H0 A :=
  ...
  | (csp_pred ?T ?P ?t) =>
      ...
      constr:(fun b:(P u) => (csp_wd T P u s b d))
  | (csr_rel ?T ?R ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let s1 := constr:(tse_int S T r1 e1)
      ...
      constr:(fun b:(R r1 r2) => (csr_wd T R u1 u2 s1 s2 b d1 d2))
  | (Ccsr_rel ?T ?R ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let s1 := constr:(tse_int S T r1 e1)
      ...
      with d1 := constr:(tse_int_wd S T r2 r1 H0 e1)
      with d2 := constr:(tse_int_wd S T r2 r1 H0 e2) in
      constr:(fun b:(R u1 u2) => (Ccsr_wd T R u1 u2 s1 s2 b d1 d2))
  | (cs_eq (c:=?T) ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let s1 := constr:(tse_int S T r1 e1)
      ...
      with d1 := constr:(tse_int_wd S T r2 r1 H0 e1)
      with d2 := constr:(tse_int_wd S T r2 r1 H0 e2) in
      constr:(fun b:(cs_eq (c:=T) u1 u2) => (eq_wd T u1 u2 s1 s2 b d1 d2))
  | (cs_ap (c:=?T) ?t1 ?t2) =>
      let e1 := tse_quote S T r1 t1 
      with e2 := tse_quote S T r1 t2 in
      let s1 := constr:(tse_int S T r1 e1)
      ...
      with d1 := constr:(tse_int_wd S T r2 r1 H0 e1)
      with d2 := constr:(tse_int_wd S T r2 r1 H0 e2) in
      constr:(fun b:(cs_ap (c:=T) u1 u2) => (ap_wd T u1 u2 s1 s2 b d1 d2))
  | (?A1 -> ?A2) =>
      let d1 := tot_set_rewr_prf1 S r1 r2 H H0 A1
...
Ltac total_csetoid_rewrite_cxt H H0 :=
  ...
  match constr:type_of_H with
  | (cs_eq (c:=?S) ?r1 ?r2) =>
      let H1 := constr:(eq_symmetric S r1 r2 H) 
      with A := type of H0 in
      let B := tot_repl_in_form S r1 r2 A
...
Ltac total_csetoid_rewrite_cxt_rev H H0 :=
  ...
  match constr:type_of_H with
  | (cs_eq (c:=?S) ?r1 ?r2) =>
      let H1 := constr:(eq_symmetric S r2 r1 H) 
      with A := type of H0 in
      let B := tot_repl_in_form S r1 r2 A
...
(** definition of another sigma type to avoid universe inconsistencies (tbd: explain) *)
...
Definition extract_correct :
  forall (t:T) (e:part_set_xexp t), 
  pse_int (forget e) t.
Proof.
...
Definition pse_int_var_inv : 
  forall t:T, pse_int pse_var t -> t = r.
Proof.
intros t h; inversion h; reflexivity.
Defined.

Definition pse_int_con_inv : forall c t:T, pse_int (pse_con c) t -> t = c.
Proof.
...
[(replacement H H0)] is the term [t2] obtained by replacing in [t1] 
...
Definition replace_in_term (T:CSetoid) (r1 r2 t1:T) 
  ...
  proj1_my_sigT T (pse_int r2 e) (replacement H H0).

Definition replace_in_term_proof (T:CSetoid) (r1 r2 t1:T) 
  (e:part_set_exp T) (H:r1[=]r2) (H0:pse_int r1 e t1) :=
  proj2_my_sigT T (pse_int r2 e) (replacement H H0).
...
Ltac Unfold_partial_csetoid_rewrite_stuff :=
  unfold replace_in_term, proj1_my_sigT, replacement, extract_correct,
...
Tactic Notation 
  "my_csetoid_rewrite" hyp(H) "in" hyp(H0) := 
